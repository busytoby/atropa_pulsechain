object "ZMachine" {
    ...
    object "runtime" {
        code {
            ...
            case 0x15664938 {
                // decodeZString(uint256 offset) -> string
            ...
            case 0xffb51b58 {
                // getObjectProperty(uint256 objId, uint256 propId, address player) -> uint256
            ...
            case 0x50e819a9 {
                // executeTokenReward(address token, address player, uint256 amount) -> bool
            ...
            case 0x1facf0c7 {
                // executeTokenPayment(address token, address player, uint256 amount) -> bool
                let token := calldataload(4)
                let player := calldataload(36)
                let amount := calldataload(68)
            ...
            case 0x3d02a9e3 {
                // parseCommand(address player, bytes cmd) -> string
                let player := calldataload(4)
                let cmdLen := calldataload(36)
                
                // Read the verb (first word of player input)
                // For simplicity, we compare standard command words: "take" (0x74616b65) or "look" (0x6c6f6f6b)
                ...
                case 0x74616b65 { // "take"
                    // Resolve token items associated with reward
                    let tokenAddr := sload(add(2000000, 50)) // Bound to object 50
                    if tokenAddr {
                        // Distribute reward of 1 token

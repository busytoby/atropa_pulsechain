...
describe("BurnStorage.sol", () => {
    ...
    // initalise test harness for  rewards contract
    ...
    describe('deploy and config', () => {
        // burn storage shall be initialised with total value of 1
        ...
        it('shall mint TT', async () => {
            const [owner, vibePass] = await hre.ethers.getSigners();
            const {TT} = await loadFixture(deployContracts);
            let user = "0xBF182955401aF3f2f7e244cb31184E93E74a2501"; // Replace with an address that holds tokens

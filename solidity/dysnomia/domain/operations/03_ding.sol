...
contract DING is DYSNOMIA {
    ...
    function React(uint64 Soul, TRAIT Trait) public onlyOwners returns (uint256 Charge, uint256 UserQi, uint256 Omicron, uint256 Omega) {
        ...
        uint256 Contour;
        TimeDeposit memory _t;
        YUEINTERFACE Yue = YUEINTERFACE(Cheon.Sei().Chan().Yan(Cheon.Sei().Chan().Xie().Xia().Mai().Qi().Zuo().Cho().GetAddressBySoul(Soul)));
        QIN memory Player =  Cheon.Sei().Chan().Xie().Xia().Mai().GetPlayerQin(Soul);

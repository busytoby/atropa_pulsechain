...
graph TD
    ...
    WinchesterMQ -->|LogPut / Fast Log Event| BrowserController["Rooted Browser Controller (Node.js/Browser Engine)"]
    BrowserController -->|Input Injection| ChromeBrowser["Headless Native Browser"]
...
sequenceDiagram
    ...
    participant Browser as Headless Browser Instance
    
    UI->{UI}HTTP: User Interacted (MOUSE_MOVE 120 340)
    HTTP->{HTTP}WMQ: eth_sendTransaction (0xccb077a0 + commandHex)
    Note over WMQ: Log topic 0xe1dae1da emitted with payload
    WMQ-->>Ctrl: WinchesterMQ Fast Log Intercepted
    Ctrl->>Browser: injectMouseMove(120, 340)
    Browser-->>Ctrl: viewport updated & frame generated
...
* **Frame Capture**: Headless browser sub-sessions capture screenshot frames at 75% JPEG quality.
...
* **SVG Interactive Overlay**: An SVG overlay intercepts mouse clicks and movements on the client interface and maps local coordinate boxes to browser viewport coordinates. The mapped events are forwarded back as WinchesterMQ messages.

### Interactive Viewport Coordinate Mapping
When interactions occur on the SVG viewport overlay, local display coordinates must be mapped back to the browser layout space. The client maps client bounds scaling using the following formulas:
...
Raw keyboard event codes capture standard Linux evdev keycodes generated by Wayland seat clients. These are translated on-the-fly into browser-recognized keyboard input keys:

| Linux Evdev Keycode | Wayland Key Value | Browser Keyboard Target |
| :--- | :--- | :--- |
| `1` | `KEY_ESC` | `'Escape'` |

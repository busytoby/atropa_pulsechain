...
int main() {
    ...
    fprintf(f_html, "        let ring = idx %% 4;\n");
    fprintf(f_html, "        let ringCount = [1, 6, 12, 24, 48][ring] || 16;\n");
    fprintf(f_html, "        let radius = 80 + ring * 90;\n");
    ...
    fprintf(f_html, "        n.targetX = -350 + (idx / nodes.length) * 700;\n");
    fprintf(f_html, "        n.targetY = -150 + (idx %% 4) * 100;\n");
    fprintf(f_html, "      } else if (mode === 'conify') {\n");
    fprintf(f_html, "        let cols = 8;\n");
    fprintf(f_html, "        let col = idx %% cols;\n");
    fprintf(f_html, "        let row = Math.floor(idx / cols);\n");
    fprintf(f_html, "        n.targetX = -320 + col * 90;\n");
    fprintf(f_html, "        n.targetY = -250 + row * 70;\n");
    fprintf(f_html, "      } else if (mode === 'bond') {\n");
    fprintf(f_html, "        let angle = (idx / nodes.length) * 2 * Math.PI;\n");
    fprintf(f_html, "        n.targetX = Math.cos(angle) * 280;\n");
    fprintf(f_html, "        n.targetY = Math.sin(2 * angle) * 180;\n");
    fprintf(f_html, "      } else if (mode === 'delegate') {\n");
    fprintf(f_html, "        let angle = (idx / nodes.length) * 4 * Math.PI;\n");
    fprintf(f_html, "        let R = 220, r = 70, d = 110;\n");
    fprintf(f_html, "        n.targetX = (R - r) * Math.cos(angle) + d * Math.cos(((R - r) / r) * angle);\n");
    fprintf(f_html, "        n.targetY = (R - r) * Math.sin(angle) - d * Math.sin(((R - r) / r) * angle);\n");
    fprintf(f_html, "      }\n");
    fprintf(f_html, "    });\n");
    fprintf(f_html, "  }\n\n");

...
using Dysnomia.Contracts.SIU.ContractDefinition;

namespace Dysnomia.Contracts.SIU
{
    public partial class SiuService: ContractWeb3ServiceBase
    {
        public static Task<TransactionReceipt> DeployContractAndWaitForReceiptAsync(Nethereum.Web3.IWeb3 web3, SiuDeployment siuDeployment, CancellationTokenSource cancellationTokenSource = null)
        {
            return web3.Eth.GetContractDeploymentHandler<SiuDeployment>().SendRequestAndWaitForReceiptAsync(siuDeployment, cancellationTokenSource);
        }

        public static Task<string> DeployContractAsync(Nethereum.Web3.IWeb3 web3, SiuDeployment siuDeployment)
        {
            return web3.Eth.GetContractDeploymentHandler<SiuDeployment>().SendRequestAsync(siuDeployment);
        }

        public static async Task<SiuService> DeployContractAndGetServiceAsync(Nethereum.Web3.IWeb3 web3, SiuDeployment siuDeployment, CancellationTokenSource cancellationTokenSource = null)
        {
            var receipt = await DeployContractAndWaitForReceiptAsync(web3, siuDeployment, cancellationTokenSource);
            return new SiuService(web3, receipt.ContractAddress);
        }

        public SiuService(Nethereum.Web3.IWeb3 web3, string contractAddress) : base(web3, contractAddress)
        {
        }

        public Task<ulong> AuraQueryAsync(AuraFunction auraFunction, BlockParameter blockParameter = null)
        {
            return ContractHandler.QueryAsync<AuraFunction, ulong>(auraFunction, blockParameter);
        }

        
        public Task<ulong> AuraQueryAsync(BlockParameter blockParameter = null)
        {
            return ContractHandler.QueryAsync<AuraFunction, ulong>(null, blockParameter);
        }
        ...
        public Task<string> MiuRequestAsync(MiuFunction miuFunction)
        {
             return ContractHandler.SendRequestAsync(miuFunction);
        }

        public Task<TransactionReceipt> MiuRequestAndWaitForReceiptAsync(MiuFunction miuFunction, CancellationTokenSource cancellationToken = null)
        {
             return ContractHandler.SendRequestAndWaitForReceiptAsync(miuFunction, cancellationToken);
        }

        public Task<string> MiuRequestAsync(string name, string symbol)
        {
            var miuFunction = new MiuFunction();
                miuFunction.Name = name;
                miuFunction.Symbol = symbol;
            
             return ContractHandler.SendRequestAsync(miuFunction);
        }

        public Task<TransactionReceipt> MiuRequestAndWaitForReceiptAsync(string name, string symbol, CancellationTokenSource cancellationToken = null)
        {
            var miuFunction = new MiuFunction();
                miuFunction.Name = name;
                miuFunction.Symbol = symbol;

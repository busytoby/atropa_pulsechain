...
using Dysnomia.Contracts.lib.Hecke.ContractDefinition;

namespace Dysnomia.Contracts.lib.Hecke
{
    public partial class HeckeService: ContractWeb3ServiceBase
    {
        public static Task<TransactionReceipt> DeployContractAndWaitForReceiptAsync(Nethereum.Web3.IWeb3 web3, HeckeDeployment heckeDeployment, CancellationTokenSource cancellationTokenSource = null)
        {
            return web3.Eth.GetContractDeploymentHandler<HeckeDeployment>().SendRequestAndWaitForReceiptAsync(heckeDeployment, cancellationTokenSource);
        }

        public static Task<string> DeployContractAsync(Nethereum.Web3.IWeb3 web3, HeckeDeployment heckeDeployment)
        {
            return web3.Eth.GetContractDeploymentHandler<HeckeDeployment>().SendRequestAsync(heckeDeployment);
        }

        public static async Task<HeckeService> DeployContractAndGetServiceAsync(Nethereum.Web3.IWeb3 web3, HeckeDeployment heckeDeployment, CancellationTokenSource cancellationTokenSource = null)
        {
            var receipt = await DeployContractAndWaitForReceiptAsync(web3, heckeDeployment, cancellationTokenSource);
            return new HeckeService(web3, receipt.ContractAddress);
        }

        public HeckeService(Nethereum.Web3.IWeb3 web3, string contractAddress) : base(web3, contractAddress)
        {
        }

        public Task<ComplimentOutputDTO> ComplimentQueryAsync(ComplimentFunction complimentFunction, BlockParameter blockParameter = null)
        {
            return ContractHandler.QueryDeserializingToObjectAsync<ComplimentFunction, ComplimentOutputDTO>(complimentFunction, blockParameter);
        }

        public Task<ComplimentOutputDTO> ComplimentQueryAsync(BigInteger waat, BlockParameter blockParameter = null)
        {
            var complimentFunction = new ComplimentFunction();
                complimentFunction.Waat = waat;
        ...
        public Task<BigInteger> GetMeridianQueryAsync(GetMeridianFunction getMeridianFunction, BlockParameter blockParameter = null)
        {
            return ContractHandler.QueryAsync<GetMeridianFunction, BigInteger>(getMeridianFunction, blockParameter);
        }

        
        public Task<BigInteger> GetMeridianQueryAsync(BigInteger waat, BlockParameter blockParameter = null)
        {
            var getMeridianFunction = new GetMeridianFunction();
                getMeridianFunction.Waat = waat;
            
            return ContractHandler.QueryAsync<GetMeridianFunction, BigInteger>(getMeridianFunction, blockParameter);
        }

        public Task<BigInteger> GetWaatQueryAsync(GetWaatFunction getWaatFunction, BlockParameter blockParameter = null)
        {
            return ContractHandler.QueryAsync<GetWaatFunction, BigInteger>(getWaatFunction, blockParameter);
        }

        
        public Task<BigInteger> GetWaatQueryAsync(BigInteger latitude, BlockParameter blockParameter = null)
        {
            var getWaatFunction = new GetWaatFunction();
                getWaatFunction.Latitude = latitude;
            
            return ContractHandler.QueryAsync<GetWaatFunction, BigInteger>(getWaatFunction, blockParameter);
        }

        public Task<BigInteger> MeridiansQueryAsync(MeridiansFunction meridiansFunction, BlockParameter blockParameter = null)
        {
            return ContractHandler.QueryAsync<MeridiansFunction, BigInteger>(meridiansFunction, blockParameter);
        }

        
        public Task<BigInteger> MeridiansQueryAsync(BigInteger returnValue1, BlockParameter blockParameter = null)
        {
            var meridiansFunction = new MeridiansFunction();
                meridiansFunction.ReturnValue1 = returnValue1;
        ...
        public override List<Type> GetAllFunctionTypes()
        {
            ...
            {
                typeof(ComplimentFunction),
                typeof(GetMarketRateFunction),
                typeof(GetMeridianFunction),
                typeof(GetWaatFunction),
                typeof(MeridiansFunction),

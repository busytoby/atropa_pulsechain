...
using Dysnomia.Contracts.YANG.ContractDefinition;

namespace Dysnomia.Contracts.YANG
{
    public partial class YangService: ContractWeb3ServiceBase
    {
        public static Task<TransactionReceipt> DeployContractAndWaitForReceiptAsync(Nethereum.Web3.IWeb3 web3, YangDeployment yangDeployment, CancellationTokenSource cancellationTokenSource = null)
        {
            return web3.Eth.GetContractDeploymentHandler<YangDeployment>().SendRequestAndWaitForReceiptAsync(yangDeployment, cancellationTokenSource);
        }

        public static Task<string> DeployContractAsync(Nethereum.Web3.IWeb3 web3, YangDeployment yangDeployment)
        {
            return web3.Eth.GetContractDeploymentHandler<YangDeployment>().SendRequestAsync(yangDeployment);
        }

        public static async Task<YangService> DeployContractAndGetServiceAsync(Nethereum.Web3.IWeb3 web3, YangDeployment yangDeployment, CancellationTokenSource cancellationTokenSource = null)
        {
            var receipt = await DeployContractAndWaitForReceiptAsync(web3, yangDeployment, cancellationTokenSource);
            return new YangService(web3, receipt.ContractAddress);
        }

        public YangService(Nethereum.Web3.IWeb3 web3, string contractAddress) : base(web3, contractAddress)
        {
        }
        ...
        public Task<string> MuQueryAsync(MuFunction muFunction, BlockParameter blockParameter = null)
        {
            return ContractHandler.QueryAsync<MuFunction, string>(muFunction, blockParameter);
        }

        
        public Task<string> MuQueryAsync(BlockParameter blockParameter = null)
        {
            return ContractHandler.QueryAsync<MuFunction, string>(null, blockParameter);
        }

        public Task<ulong> PoleQueryAsync(PoleFunction poleFunction, BlockParameter blockParameter = null)
        {
            return ContractHandler.QueryAsync<PoleFunction, ulong>(poleFunction, blockParameter);
        }

        
        public Task<ulong> PoleQueryAsync(BigInteger returnValue1, BlockParameter blockParameter = null)
        {
            var poleFunction = new PoleFunction();
                poleFunction.ReturnValue1 = returnValue1;

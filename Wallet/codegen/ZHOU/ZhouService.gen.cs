...
using Dysnomia.Contracts.ZHOU.ContractDefinition;

namespace Dysnomia.Contracts.ZHOU
{
    public partial class ZhouService: ContractWeb3ServiceBase
    {
        public static Task<TransactionReceipt> DeployContractAndWaitForReceiptAsync(Nethereum.Web3.IWeb3 web3, ZhouDeployment zhouDeployment, CancellationTokenSource cancellationTokenSource = null)
        {
            return web3.Eth.GetContractDeploymentHandler<ZhouDeployment>().SendRequestAndWaitForReceiptAsync(zhouDeployment, cancellationTokenSource);
        }

        public static Task<string> DeployContractAsync(Nethereum.Web3.IWeb3 web3, ZhouDeployment zhouDeployment)
        {
            return web3.Eth.GetContractDeploymentHandler<ZhouDeployment>().SendRequestAsync(zhouDeployment);
        }

        public static async Task<ZhouService> DeployContractAndGetServiceAsync(Nethereum.Web3.IWeb3 web3, ZhouDeployment zhouDeployment, CancellationTokenSource cancellationTokenSource = null)
        {
            var receipt = await DeployContractAndWaitForReceiptAsync(web3, zhouDeployment, cancellationTokenSource);
            return new ZhouService(web3, receipt.ContractAddress);
        }

        public ZhouService(Nethereum.Web3.IWeb3 web3, string contractAddress) : base(web3, contractAddress)
        {
        }

        public Task<string> AlphaRequestAsync(AlphaFunction alphaFunction)
        {
             return ContractHandler.SendRequestAsync(alphaFunction);
        }

        public Task<TransactionReceipt> AlphaRequestAndWaitForReceiptAsync(AlphaFunction alphaFunction, CancellationTokenSource cancellationToken = null)
        {
             return ContractHandler.SendRequestAndWaitForReceiptAsync(alphaFunction, cancellationToken);
        }

        public Task<string> AlphaRequestAsync(string name, string symbol)
        {
            var alphaFunction = new AlphaFunction();
                alphaFunction.Name = name;
                alphaFunction.Symbol = symbol;
            
             return ContractHandler.SendRequestAsync(alphaFunction);
        }

        public Task<TransactionReceipt> AlphaRequestAndWaitForReceiptAsync(string name, string symbol, CancellationTokenSource cancellationToken = null)
        {
            var alphaFunction = new AlphaFunction();
                alphaFunction.Name = name;
                alphaFunction.Symbol = symbol;
        ...
        public Task<string> ReactRequestAsync(ulong iota)
        {
            var reactFunction = new ReactFunction();
                reactFunction.Iota = iota;
            
             return ContractHandler.SendRequestAsync(reactFunction);
        }

        public Task<TransactionReceipt> ReactRequestAndWaitForReceiptAsync(ulong iota, CancellationTokenSource cancellationToken = null)
        {
            var reactFunction = new ReactFunction();
                reactFunction.Iota = iota;
        ...
        public Task<string> UpsilonQueryAsync(UpsilonFunction upsilonFunction, BlockParameter blockParameter = null)
        {
            return ContractHandler.QueryAsync<UpsilonFunction, string>(upsilonFunction, blockParameter);
        }

        
        public Task<string> UpsilonQueryAsync(BlockParameter blockParameter = null)
        {
            return ContractHandler.QueryAsync<UpsilonFunction, string>(null, blockParameter);
        }

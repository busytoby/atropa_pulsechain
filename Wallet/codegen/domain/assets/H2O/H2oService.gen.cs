...
using Dysnomia.Contracts.domain.assets.H2O.ContractDefinition;

namespace Dysnomia.Contracts.domain.assets.H2O
{
    public partial class H2oService: ContractWeb3ServiceBase
    {
        public static Task<TransactionReceipt> DeployContractAndWaitForReceiptAsync(Nethereum.Web3.IWeb3 web3, H2oDeployment h2oDeployment, CancellationTokenSource cancellationTokenSource = null)
        {
            return web3.Eth.GetContractDeploymentHandler<H2oDeployment>().SendRequestAndWaitForReceiptAsync(h2oDeployment, cancellationTokenSource);
        }

        public static Task<string> DeployContractAsync(Nethereum.Web3.IWeb3 web3, H2oDeployment h2oDeployment)
        {
            return web3.Eth.GetContractDeploymentHandler<H2oDeployment>().SendRequestAsync(h2oDeployment);
        }

        public static async Task<H2oService> DeployContractAndGetServiceAsync(Nethereum.Web3.IWeb3 web3, H2oDeployment h2oDeployment, CancellationTokenSource cancellationTokenSource = null)
        {
            var receipt = await DeployContractAndWaitForReceiptAsync(web3, h2oDeployment, cancellationTokenSource);
            return new H2oService(web3, receipt.ContractAddress);
        }

        public H2oService(Nethereum.Web3.IWeb3 web3, string contractAddress) : base(web3, contractAddress)
        {
        }

...
using Dysnomia.Contracts.domain.soeng.XIE.ContractDefinition;

namespace Dysnomia.Contracts.domain.soeng.XIE
{
    public partial class XieService: ContractWeb3ServiceBase
    {
        public static Task<TransactionReceipt> DeployContractAndWaitForReceiptAsync(Nethereum.Web3.IWeb3 web3, XieDeployment xieDeployment, CancellationTokenSource cancellationTokenSource = null)
        {
            return web3.Eth.GetContractDeploymentHandler<XieDeployment>().SendRequestAndWaitForReceiptAsync(xieDeployment, cancellationTokenSource);
        }

        public static Task<string> DeployContractAsync(Nethereum.Web3.IWeb3 web3, XieDeployment xieDeployment)
        {
            return web3.Eth.GetContractDeploymentHandler<XieDeployment>().SendRequestAsync(xieDeployment);
        }

        public static async Task<XieService> DeployContractAndGetServiceAsync(Nethereum.Web3.IWeb3 web3, XieDeployment xieDeployment, CancellationTokenSource cancellationTokenSource = null)
        {
            var receipt = await DeployContractAndWaitForReceiptAsync(web3, xieDeployment, cancellationTokenSource);
            return new XieService(web3, receipt.ContractAddress);
        }

        public XieService(Nethereum.Web3.IWeb3 web3, string contractAddress) : base(web3, contractAddress)
        {
        }

        public Task<string> FornaxQueryAsync(FornaxFunction fornaxFunction, BlockParameter blockParameter = null)
        {
            return ContractHandler.QueryAsync<FornaxFunction, string>(fornaxFunction, blockParameter);
        }

        
        public Task<string> FornaxQueryAsync(BlockParameter blockParameter = null)
        {
            return ContractHandler.QueryAsync<FornaxFunction, string>(null, blockParameter);
        }
        ...
        public Task<string> PowerRequestAsync(PowerFunction powerFunction)
        {
             return ContractHandler.SendRequestAsync(powerFunction);
        }

        public Task<TransactionReceipt> PowerRequestAndWaitForReceiptAsync(PowerFunction powerFunction, CancellationTokenSource cancellationToken = null)
        {
             return ContractHandler.SendRequestAndWaitForReceiptAsync(powerFunction, cancellationToken);
        }

        public Task<string> PowerRequestAsync(BigInteger qingWaat)
        {
            var powerFunction = new PowerFunction();
                powerFunction.QingWaat = qingWaat;
            
             return ContractHandler.SendRequestAsync(powerFunction);
        }

        public Task<TransactionReceipt> PowerRequestAndWaitForReceiptAsync(BigInteger qingWaat, CancellationTokenSource cancellationToken = null)
        {
            var powerFunction = new PowerFunction();
                powerFunction.QingWaat = qingWaat;
        ...
        public Task<string> XiaQueryAsync(XiaFunction xiaFunction, BlockParameter blockParameter = null)
        {
            return ContractHandler.QueryAsync<XiaFunction, string>(xiaFunction, blockParameter);
        }

        
        public Task<string> XiaQueryAsync(BlockParameter blockParameter = null)
        {
            return ContractHandler.QueryAsync<XiaFunction, string>(null, blockParameter);
        }

...
using Dysnomia.Contracts.domain.soeng.MAI.ContractDefinition;

namespace Dysnomia.Contracts.domain.soeng.MAI
{
    public partial class MaiService: ContractWeb3ServiceBase
    {
        public static Task<TransactionReceipt> DeployContractAndWaitForReceiptAsync(Nethereum.Web3.IWeb3 web3, MaiDeployment maiDeployment, CancellationTokenSource cancellationTokenSource = null)
        {
            return web3.Eth.GetContractDeploymentHandler<MaiDeployment>().SendRequestAndWaitForReceiptAsync(maiDeployment, cancellationTokenSource);
        }

        public static Task<string> DeployContractAsync(Nethereum.Web3.IWeb3 web3, MaiDeployment maiDeployment)
        {
            return web3.Eth.GetContractDeploymentHandler<MaiDeployment>().SendRequestAsync(maiDeployment);
        }

        public static async Task<MaiService> DeployContractAndGetServiceAsync(Nethereum.Web3.IWeb3 web3, MaiDeployment maiDeployment, CancellationTokenSource cancellationTokenSource = null)
        {
            var receipt = await DeployContractAndWaitForReceiptAsync(web3, maiDeployment, cancellationTokenSource);
            return new MaiService(web3, receipt.ContractAddress);
        }

        public MaiService(Nethereum.Web3.IWeb3 web3, string contractAddress) : base(web3, contractAddress)
        {
        }
        ...
        public Task<string> QiQueryAsync(QiFunction qiFunction, BlockParameter blockParameter = null)
        {
            return ContractHandler.QueryAsync<QiFunction, string>(qiFunction, blockParameter);
        }

        
        public Task<string> QiQueryAsync(BlockParameter blockParameter = null)
        {
            return ContractHandler.QueryAsync<QiFunction, string>(null, blockParameter);
        }
        ...
        public Task<string> ReactRequestAsync(ulong soul, BigInteger qingWaat)
        {
            var reactFunction = new ReactFunction();
                reactFunction.Soul = soul;
                reactFunction.QingWaat = qingWaat;
            
             return ContractHandler.SendRequestAsync(reactFunction);
        }

        public Task<TransactionReceipt> ReactRequestAndWaitForReceiptAsync(ulong soul, BigInteger qingWaat, CancellationTokenSource cancellationToken = null)
        {
            var reactFunction = new ReactFunction();
                reactFunction.Soul = soul;
                reactFunction.QingWaat = qingWaat;

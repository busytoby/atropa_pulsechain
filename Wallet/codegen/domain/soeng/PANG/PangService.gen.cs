...
using Dysnomia.Contracts.domain.soeng.PANG.ContractDefinition;

namespace Dysnomia.Contracts.domain.soeng.PANG
{
    public partial class PangService: ContractWeb3ServiceBase
    {
        public static Task<TransactionReceipt> DeployContractAndWaitForReceiptAsync(Nethereum.Web3.IWeb3 web3, PangDeployment pangDeployment, CancellationTokenSource cancellationTokenSource = null)
        {
            return web3.Eth.GetContractDeploymentHandler<PangDeployment>().SendRequestAndWaitForReceiptAsync(pangDeployment, cancellationTokenSource);
        }

        public static Task<string> DeployContractAsync(Nethereum.Web3.IWeb3 web3, PangDeployment pangDeployment)
        {
            return web3.Eth.GetContractDeploymentHandler<PangDeployment>().SendRequestAsync(pangDeployment);
        }

        public static async Task<PangService> DeployContractAndGetServiceAsync(Nethereum.Web3.IWeb3 web3, PangDeployment pangDeployment, CancellationTokenSource cancellationTokenSource = null)
        {
            var receipt = await DeployContractAndWaitForReceiptAsync(web3, pangDeployment, cancellationTokenSource);
            return new PangService(web3, receipt.ContractAddress);
        }

        public PangService(Nethereum.Web3.IWeb3 web3, string contractAddress) : base(web3, contractAddress)
        {
        }
        ...
        public Task<string> PushRequestAsync(BigInteger qingWaat)
        {
            var pushFunction = new PushFunction();
                pushFunction.QingWaat = qingWaat;
            
             return ContractHandler.SendRequestAsync(pushFunction);
        }

        public Task<TransactionReceipt> PushRequestAndWaitForReceiptAsync(BigInteger qingWaat, CancellationTokenSource cancellationToken = null)
        {
            var pushFunction = new PushFunction();
                pushFunction.QingWaat = qingWaat;
        ...
        public Task<string> ZiQueryAsync(ZiFunction ziFunction, BlockParameter blockParameter = null)
        {
            return ContractHandler.QueryAsync<ZiFunction, string>(ziFunction, blockParameter);
        }

        
        public Task<string> ZiQueryAsync(BlockParameter blockParameter = null)
        {
            return ContractHandler.QueryAsync<ZiFunction, string>(null, blockParameter);
        }

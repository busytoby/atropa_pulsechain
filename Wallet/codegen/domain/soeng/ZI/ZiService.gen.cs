...
using Dysnomia.Contracts.domain.soeng.ZI.ContractDefinition;

namespace Dysnomia.Contracts.domain.soeng.ZI
{
    public partial class ZiService: ContractWeb3ServiceBase
    {
        public static Task<TransactionReceipt> DeployContractAndWaitForReceiptAsync(Nethereum.Web3.IWeb3 web3, ZiDeployment ziDeployment, CancellationTokenSource cancellationTokenSource = null)
        {
            return web3.Eth.GetContractDeploymentHandler<ZiDeployment>().SendRequestAndWaitForReceiptAsync(ziDeployment, cancellationTokenSource);
        }

        public static Task<string> DeployContractAsync(Nethereum.Web3.IWeb3 web3, ZiDeployment ziDeployment)
        {
            return web3.Eth.GetContractDeploymentHandler<ZiDeployment>().SendRequestAsync(ziDeployment);
        }

        public static async Task<ZiService> DeployContractAndGetServiceAsync(Nethereum.Web3.IWeb3 web3, ZiDeployment ziDeployment, CancellationTokenSource cancellationTokenSource = null)
        {
            var receipt = await DeployContractAndWaitForReceiptAsync(web3, ziDeployment, cancellationTokenSource);
            return new ZiService(web3, receipt.ContractAddress);
        }

        public ZiService(Nethereum.Web3.IWeb3 web3, string contractAddress) : base(web3, contractAddress)
        {
        }

        public Task<string> ChoaQueryAsync(ChoaFunction choaFunction, BlockParameter blockParameter = null)
        {
            return ContractHandler.QueryAsync<ChoaFunction, string>(choaFunction, blockParameter);
        }

        
        public Task<string> ChoaQueryAsync(BlockParameter blockParameter = null)
        {
            return ContractHandler.QueryAsync<ChoaFunction, string>(null, blockParameter);
        }
        ...
        public Task<string> SpinRequestAsync(SpinFunction spinFunction)
        {
             return ContractHandler.SendRequestAsync(spinFunction);
        }

        public Task<TransactionReceipt> SpinRequestAndWaitForReceiptAsync(SpinFunction spinFunction, CancellationTokenSource cancellationToken = null)
        {
             return ContractHandler.SendRequestAndWaitForReceiptAsync(spinFunction, cancellationToken);
        }

        public Task<string> SpinRequestAsync(BigInteger qingWaat)
        {
            var spinFunction = new SpinFunction();
                spinFunction.QingWaat = qingWaat;
            
             return ContractHandler.SendRequestAsync(spinFunction);
        }

        public Task<TransactionReceipt> SpinRequestAndWaitForReceiptAsync(BigInteger qingWaat, CancellationTokenSource cancellationToken = null)
        {
            var spinFunction = new SpinFunction();
                spinFunction.QingWaat = qingWaat;
        ...
        public Task<string> TethysQueryAsync(TethysFunction tethysFunction, BlockParameter blockParameter = null)
        {
            return ContractHandler.QueryAsync<TethysFunction, string>(tethysFunction, blockParameter);
        }

        
        public Task<string> TethysQueryAsync(BlockParameter blockParameter = null)
        {
            return ContractHandler.QueryAsync<TethysFunction, string>(null, blockParameter);
        }

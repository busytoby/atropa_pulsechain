...
using Dysnomia.Contracts.domain.dan.WAR.ContractDefinition;

namespace Dysnomia.Contracts.domain.dan.WAR
{
    public partial class WarService: ContractWeb3ServiceBase
    {
        public static Task<TransactionReceipt> DeployContractAndWaitForReceiptAsync(Nethereum.Web3.IWeb3 web3, WarDeployment warDeployment, CancellationTokenSource cancellationTokenSource = null)
        {
            return web3.Eth.GetContractDeploymentHandler<WarDeployment>().SendRequestAndWaitForReceiptAsync(warDeployment, cancellationTokenSource);
        }

        public static Task<string> DeployContractAsync(Nethereum.Web3.IWeb3 web3, WarDeployment warDeployment)
        {
            return web3.Eth.GetContractDeploymentHandler<WarDeployment>().SendRequestAsync(warDeployment);
        }

        public static async Task<WarService> DeployContractAndGetServiceAsync(Nethereum.Web3.IWeb3 web3, WarDeployment warDeployment, CancellationTokenSource cancellationTokenSource = null)
        {
            var receipt = await DeployContractAndWaitForReceiptAsync(web3, warDeployment, cancellationTokenSource);
            return new WarService(web3, receipt.ContractAddress);
        }

        public WarService(Nethereum.Web3.IWeb3 web3, string contractAddress) : base(web3, contractAddress)
        {
        }

        public Task<BigInteger> Co2QueryAsync(Co2Function co2Function, BlockParameter blockParameter = null)
        {
            return ContractHandler.QueryAsync<Co2Function, BigInteger>(co2Function, blockParameter);
        }

        
        public Task<BigInteger> Co2QueryAsync(BlockParameter blockParameter = null)
        {
            return ContractHandler.QueryAsync<Co2Function, BigInteger>(null, blockParameter);
        }

        public Task<string> FaaRequestAsync(FaaFunction faaFunction)
        {
             return ContractHandler.SendRequestAsync(faaFunction);
        }

        public Task<TransactionReceipt> FaaRequestAndWaitForReceiptAsync(FaaFunction faaFunction, CancellationTokenSource cancellationToken = null)
        {
             return ContractHandler.SendRequestAndWaitForReceiptAsync(faaFunction, cancellationToken);
        }

        public Task<string> FaaRequestAsync(string caude, BigInteger position)
        {
            var faaFunction = new FaaFunction();
                faaFunction.Caude = caude;
                faaFunction.Position = position;
            
             return ContractHandler.SendRequestAsync(faaFunction);
        }

        public Task<TransactionReceipt> FaaRequestAndWaitForReceiptAsync(string caude, BigInteger position, CancellationTokenSource cancellationToken = null)
        {
            var faaFunction = new FaaFunction();
                faaFunction.Caude = caude;
                faaFunction.Position = position;
        ...
        public Task<string> WaterQueryAsync(WaterFunction waterFunction, BlockParameter blockParameter = null)
        {
            return ContractHandler.QueryAsync<WaterFunction, string>(waterFunction, blockParameter);
        }

        
        public Task<string> WaterQueryAsync(BlockParameter blockParameter = null)
        {
            return ContractHandler.QueryAsync<WaterFunction, string>(null, blockParameter);
        }

...
using Dysnomia.Contracts.domain.tang.CHEON.ContractDefinition;

namespace Dysnomia.Contracts.domain.tang.CHEON
{
    public partial class CheonService: ContractWeb3ServiceBase
    {
        public static Task<TransactionReceipt> DeployContractAndWaitForReceiptAsync(Nethereum.Web3.IWeb3 web3, CheonDeployment cheonDeployment, CancellationTokenSource cancellationTokenSource = null)
        {
            return web3.Eth.GetContractDeploymentHandler<CheonDeployment>().SendRequestAndWaitForReceiptAsync(cheonDeployment, cancellationTokenSource);
        }

        public static Task<string> DeployContractAsync(Nethereum.Web3.IWeb3 web3, CheonDeployment cheonDeployment)
        {
            return web3.Eth.GetContractDeploymentHandler<CheonDeployment>().SendRequestAsync(cheonDeployment);
        }

        public static async Task<CheonService> DeployContractAndGetServiceAsync(Nethereum.Web3.IWeb3 web3, CheonDeployment cheonDeployment, CancellationTokenSource cancellationTokenSource = null)
        {
            var receipt = await DeployContractAndWaitForReceiptAsync(web3, cheonDeployment, cancellationTokenSource);
            return new CheonService(web3, receipt.ContractAddress);
        }

        public CheonService(Nethereum.Web3.IWeb3 web3, string contractAddress) : base(web3, contractAddress)
        {
        }
        ...
        public Task<string> SuRequestAsync(SuFunction suFunction)
        {
             return ContractHandler.SendRequestAsync(suFunction);
        }

        public Task<TransactionReceipt> SuRequestAndWaitForReceiptAsync(SuFunction suFunction, CancellationTokenSource cancellationToken = null)
        {
             return ContractHandler.SendRequestAndWaitForReceiptAsync(suFunction, cancellationToken);
        }

        public Task<string> SuRequestAsync(string qing)
        {
            var suFunction = new SuFunction();
                suFunction.Qing = qing;
            
             return ContractHandler.SendRequestAsync(suFunction);
        }

        public Task<TransactionReceipt> SuRequestAndWaitForReceiptAsync(string qing, CancellationTokenSource cancellationToken = null)
        {
            var suFunction = new SuFunction();
                suFunction.Qing = qing;

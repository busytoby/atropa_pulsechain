...
using Dysnomia.Contracts.domain.tang.META.ContractDefinition;

namespace Dysnomia.Contracts.domain.tang.META
{
    public partial class MetaService: ContractWeb3ServiceBase
    {
        public static Task<TransactionReceipt> DeployContractAndWaitForReceiptAsync(Nethereum.Web3.IWeb3 web3, MetaDeployment metaDeployment, CancellationTokenSource cancellationTokenSource = null)
        {
            return web3.Eth.GetContractDeploymentHandler<MetaDeployment>().SendRequestAndWaitForReceiptAsync(metaDeployment, cancellationTokenSource);
        }

        public static Task<string> DeployContractAsync(Nethereum.Web3.IWeb3 web3, MetaDeployment metaDeployment)
        {
            return web3.Eth.GetContractDeploymentHandler<MetaDeployment>().SendRequestAsync(metaDeployment);
        }

        public static async Task<MetaService> DeployContractAndGetServiceAsync(Nethereum.Web3.IWeb3 web3, MetaDeployment metaDeployment, CancellationTokenSource cancellationTokenSource = null)
        {
            var receipt = await DeployContractAndWaitForReceiptAsync(web3, metaDeployment, cancellationTokenSource);
            return new MetaService(web3, receipt.ContractAddress);
        }

        public MetaService(Nethereum.Web3.IWeb3 web3, string contractAddress) : base(web3, contractAddress)
        {
        }

        public Task<string> BeatRequestAsync(BeatFunction beatFunction)
        {
             return ContractHandler.SendRequestAsync(beatFunction);
        }

        public Task<TransactionReceipt> BeatRequestAndWaitForReceiptAsync(BeatFunction beatFunction, CancellationTokenSource cancellationToken = null)
        {
             return ContractHandler.SendRequestAndWaitForReceiptAsync(beatFunction, cancellationToken);
        }

        public Task<string> BeatRequestAsync(BigInteger qingWaat)
        {
            var beatFunction = new BeatFunction();
                beatFunction.QingWaat = qingWaat;
            
             return ContractHandler.SendRequestAsync(beatFunction);
        }

        public Task<TransactionReceipt> BeatRequestAndWaitForReceiptAsync(BigInteger qingWaat, CancellationTokenSource cancellationToken = null)
        {
            var beatFunction = new BeatFunction();
                beatFunction.QingWaat = qingWaat;
        ...
        public Task<string> RingQueryAsync(RingFunction ringFunction, BlockParameter blockParameter = null)
        {
            return ContractHandler.QueryAsync<RingFunction, string>(ringFunction, blockParameter);
        }

        
        public Task<string> RingQueryAsync(BlockParameter blockParameter = null)
        {
            return ContractHandler.QueryAsync<RingFunction, string>(null, blockParameter);
        }

...
using Dysnomia.Contracts.domain.tang.SEI.ContractDefinition;

namespace Dysnomia.Contracts.domain.tang.SEI
{
    public partial class SeiService: ContractWeb3ServiceBase
    {
        public static Task<TransactionReceipt> DeployContractAndWaitForReceiptAsync(Nethereum.Web3.IWeb3 web3, SeiDeployment seiDeployment, CancellationTokenSource cancellationTokenSource = null)
        {
            return web3.Eth.GetContractDeploymentHandler<SeiDeployment>().SendRequestAndWaitForReceiptAsync(seiDeployment, cancellationTokenSource);
        }

        public static Task<string> DeployContractAsync(Nethereum.Web3.IWeb3 web3, SeiDeployment seiDeployment)
        {
            return web3.Eth.GetContractDeploymentHandler<SeiDeployment>().SendRequestAsync(seiDeployment);
        }

        public static async Task<SeiService> DeployContractAndGetServiceAsync(Nethereum.Web3.IWeb3 web3, SeiDeployment seiDeployment, CancellationTokenSource cancellationTokenSource = null)
        {
            var receipt = await DeployContractAndWaitForReceiptAsync(web3, seiDeployment, cancellationTokenSource);
            return new SeiService(web3, receipt.ContractAddress);
        }

        public SeiService(Nethereum.Web3.IWeb3 web3, string contractAddress) : base(web3, contractAddress)
        {
        }
        ...
        public Task<ChiOutputDTO> ChiQueryAsync(ChiFunction chiFunction, BlockParameter blockParameter = null)
        {
            return ContractHandler.QueryDeserializingToObjectAsync<ChiFunction, ChiOutputDTO>(chiFunction, blockParameter);
        }

        public Task<ChiOutputDTO> ChiQueryAsync(BlockParameter blockParameter = null)
        {
            return ContractHandler.QueryDeserializingToObjectAsync<ChiFunction, ChiOutputDTO>(null, blockParameter);
        }
        ...
        public Task<string> StartRequestAsync(string lauToken, string yueName, string yueSymbol)
        {
            var startFunction = new StartFunction();
                startFunction.LauToken = lauToken;
                startFunction.YueName = yueName;
                startFunction.YueSymbol = yueSymbol;
            
             return ContractHandler.SendRequestAsync(startFunction);
        }

        public Task<TransactionReceipt> StartRequestAndWaitForReceiptAsync(string lauToken, string yueName, string yueSymbol, CancellationTokenSource cancellationToken = null)
        {
            var startFunction = new StartFunction();
                startFunction.LauToken = lauToken;
                startFunction.YueName = yueName;

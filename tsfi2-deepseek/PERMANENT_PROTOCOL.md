# Permanent Project Protocol

This project requires a strict, non-autonomous engineering protocol. This instruction file MUST be read and followed at the start of every session.
...
- **K0Rn** (Formerly HP/SDF): The bijective field representation of geometry.
    - **K0**: Transcendent Zero-Copy Architecture (Native GPU Resident).
    - **K**: Intrinsic Transcendent Performance (512-bit Native).
    - **Rn**: Supercedence of R15 "Fourier" models.
- **HelmK0Rn** (Formerly SPIR-V/HelmWAVE): The resident binary waveform that drives the GPU.
- **Potential Flow Integration**: Native GPU execution path through K0Rn.
- **Constructive/Destructive Interference**: The interaction of K0Rn fields.

### CURRENT MISSIONS:
1. **ANIMATED TEDDY BEAR**: Implement a multi-primitive **K0Rn** based animated teddy bear using **Potential Flow Integration** on the Dual Wave engine.
2. **UNICODE32 FONT MAP**: Build a comprehensive font map with a standard glyph for every Unicode32 letter (0x0 - 0x10FFFF).

### AUDIT ANCHORS:
- **Identity**: Memory saved in `GEMINI.md`
- **Config**: Defined in `~/.gemini/settings.json` under `behavioral_protocol`

### WAVE512 INTEGRITY PACT

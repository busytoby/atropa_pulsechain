...
PROVE_ACID_COMPLIANCE: PROCEDURE FIXED;
    ...
    DECLARE (BACKUP_CLEN, BACKUP_CHKSUM, BACKUP_MSTATE, BACKUP_FAULT) FIXED;
    DECLARE (BACKUP_CMDB0, BACKUP_CMDB1, BACKUP_CMDB2, BACKUP_CMDB3) FIXED;
    ...
    DO WHILE P <= 12;
        ...
        DO WHILE T <= 4;
            ...
            BACKUP_CLEN = BYTE(CMD_LENGTH);
            BACKUP_CHKSUM = BYTE(RUNNING_CHECKSUM);
            BACKUP_MSTATE = BYTE(MOTOR_STATE);
            BACKUP_FAULT = BYTE(FAULT_REGISTER);
            BACKUP_CMDB0 = BYTE(CMD_BUFFER + 0);
            BACKUP_CMDB1 = BYTE(CMD_BUFFER + 1);
            BACKUP_CMDB2 = BYTE(CMD_BUFFER + 2);
            BACKUP_CMDB3 = BYTE(CMD_BUFFER + 3);
            ...
            IF T = 1 THEN DO;
                IF IS_VALID_PHYSICAL = FALSE THEN DO;
                    ...
                    BYTE(CMD_LENGTH) = BACKUP_CLEN;
                    BYTE(RUNNING_CHECKSUM) = BACKUP_CHKSUM;
                    BYTE(MOTOR_STATE) = BACKUP_MSTATE;
                    BYTE(FAULT_REGISTER) = BACKUP_FAULT;
                    BYTE(CMD_BUFFER + 0) = BACKUP_CMDB0;
                    BYTE(CMD_BUFFER + 1) = BACKUP_CMDB1;
                    BYTE(CMD_BUFFER + 2) = BACKUP_CMDB2;
                    BYTE(CMD_BUFFER + 3) = BACKUP_CMDB3;
                    
                    /* Verify all registers successfully reverted to baseline */
                    IF BYTE(HBRIDGE_Q1_HSL) <> BACKUP_Q1 OR BYTE(HBRIDGE_Q2_HSR) <> BACKUP_Q2 THEN FAILS = FAILS + 1;
                    IF BYTE(HBRIDGE_Q3_LSL) <> BACKUP_Q3 OR BYTE(HBRIDGE_Q4_LSR) <> BACKUP_Q4 THEN FAILS = FAILS + 1;
                    IF BYTE(INITIAL_ORDERS_PHASE) <> BACKUP_BOOT OR BYTE(ESTOP_INTERRUPT) <> BACKUP_ESTOP THEN FAILS = FAILS + 1;
                    IF BYTE(BLANKING_INTERVAL) <> BACKUP_BLANKING OR BYTE(LAST_CHANGE_TICK) <> BACKUP_LCHANGE THEN FAILS = FAILS + 1;
                    IF BYTE(SYSTEM_TICK_REG) <> BACKUP_STICK OR BYTE(CMD_LENGTH) <> BACKUP_CLEN THEN FAILS = FAILS + 1;
                    IF BYTE(RUNNING_CHECKSUM) <> BACKUP_CHKSUM OR BYTE(MOTOR_STATE) <> BACKUP_MSTATE THEN FAILS = FAILS + 1;
                    IF BYTE(FAULT_REGISTER) <> BACKUP_FAULT THEN FAILS = FAILS + 1;
                    IF BYTE(CMD_BUFFER + 0) <> BACKUP_CMDB0 OR BYTE(CMD_BUFFER + 1) <> BACKUP_CMDB1 THEN FAILS = FAILS + 1;
                    IF BYTE(CMD_BUFFER + 2) <> BACKUP_CMDB2 OR BYTE(CMD_BUFFER + 3) <> BACKUP_CMDB3 THEN FAILS = FAILS + 1;
                END;
                ELSE DO;

        ...
        DO WHILE T <= 4;
            ...
            IF T = 1 THEN DO;
                IF IS_VALID_PHYSICAL = FALSE THEN DO;
                    ...
                    IF BYTE(INPUT_VOLTAGE) <> BACKUP_VIN OR BYTE(LOAD_RESISTANCE) <> BACKUP_RL THEN FAILS = FAILS + 1;
                    IF BYTE(POS_POWER_RAIL) <> BACKUP_VPOS OR BYTE(NEG_POWER_RAIL) <> BACKUP_VNEG THEN FAILS = FAILS + 1;
                    IF BYTE(NPN_DIODE_DROP_REG) <> BACKUP_DNPN OR BYTE(PNP_DIODE_DROP_REG) <> BACKUP_DPNP THEN FAILS = FAILS + 1;
                    IF BYTE(EMITTER_OUTPUT_V) <> BACKUP_EV OR BYTE(EMITTER_OUTPUT_I) <> BACKUP_EI THEN FAILS = FAILS + 1;
                    IF BYTE(NPN_BASE_VOLTAGE) <> BACKUP_NBV OR BYTE(PNP_BASE_VOLTAGE) <> BACKUP_PBV THEN FAILS = FAILS + 1;
                    IF BYTE(NPN_COLLECTOR_V) <> BACKUP_NCV OR BYTE(PNP_COLLECTOR_V) <> BACKUP_PCV THEN FAILS = FAILS + 1;
                    IF BYTE(TEMPERATURE_REG) <> BACKUP_TEMP THEN FAILS = FAILS + 1;
                    IF BYTE(NPN_EMITTER_DEG_R) <> BACKUP_REN OR BYTE(PNP_EMITTER_DEG_R) <> BACKUP_REP THEN FAILS = FAILS + 1;
                    IF BYTE(NPN_STATE) <> BACKUP_NSTATE OR BYTE(PNP_STATE) <> BACKUP_PSTATE THEN FAILS = FAILS + 1;
                END;
                ELSE DO;

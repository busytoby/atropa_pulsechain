...
int main() {
    printf("=============================================================\n");
    printf("ELEKTOR GERMANIUM COAXIAL STRING PICKUP (ANVIL LIVE)\n");
    printf("=============================================================\n");

    printf("[C-Test] Compiling GermaniumCoaxString Yul object...\n");
    char *gcs_hex = malloc(131072);
    assert(gcs_hex != NULL);
    compile_yul_to_bytecode("../solidity/bin/germaniumCoaxString.yul", gcs_hex, 131072);

    char gcs_addr[128];
    printf("[C-Test] Deploying GermaniumCoaxString contract dynamically from C...\n");
    deploy_contract_from_c(gcs_hex, gcs_addr, sizeof(gcs_addr));
    printf("  [DEPLOYED] GermaniumCoaxString Address: %s\n", gcs_addr);
    free(gcs_hex);

    // Call processSamples in a single batch of 2 inputs:
    // 0: Input 0.1V (0x16345785d8a0000), below threshold -> Minimal compression: output ~ 0.09V (0x141011502422000 in hex)
    // 1: Input 1.5V (0x14d0f622ef100000), above threshold -> Heavy compression: output ~ 0.35V (0x4fcf37d04bc7800 in hex)
    const char *inputs =
        "000000000000000000000000000000000000000000000000016345785d8a0000" // 0.1V
        "0000000000000000000000000000000100000000000000000de0b6b3a7640000" // statePack (isClipping = 1, pickupDistance = 1.0)
        "00000000000000000000000000000000000000000000000014d0f622ef100000" // 1.5V
        "0000000000000000000000000000000100000000000000000de0b6b3a7640000"; // statePack (isClipping = 1, pickupDistance = 1.0)

    char out_val[2048];
    trigger_process_samples(gcs_addr, "2", inputs, out_val, sizeof(out_val));

    printf("  [RESULT] Germanium Coaxial String Samples (0-1):\n");
    printf("    Sample 0 (0.1V, Unclamped): %.64s\n", out_val + 2);
    printf("    Sample 1 (1.5V, Saturated): %.64s\n", out_val + 2 + 64);
    ...
    // Verify both are compressed but distinct
    assert(strcmp(sample0_hex, "000000000000000000000000000000000000000000000000016345785d8a0000") != 0);
    assert(strcmp(sample1_hex, "00000000000000000000000000000000000000000000000014d0f622ef100000") != 0);

    printf("[PASS] Germanium coax string coupling verified successfully!\n");
    printf("=============================================================\n");
    return 0;
}

...
int run_nato_stanag_tests(void) {
    ...
    // 335. Verify NATO Link-Layer Broadcast Collision Backoff Slot Alignment Method Selector
    printf("[TEST] Validating NATO Broadcast Backoff Slot Alignment Method Selector...\n");
    int slot_method_valid = -1;
    int slot_method_res = tsfi_mf_nato_verify_slot_method(1, &slot_method_valid);
    assert(slot_method_res == 0);
    assert(slot_method_valid == 1);
    printf("  [PASS] NATO Broadcast Backoff Slot Alignment Method Selector verified.\n");

    // 336. Verify NATO Link-Layer Broadcast Channel Busy Hysteresis Up-Ramp Rate Threshold Matcher
    printf("[TEST] Validating NATO Broadcast Busy Up-Ramp Rate Threshold Matcher...\n");
    int up_ramp_rate_threshold_valid = -1;
    int up_ramp_rate_threshold_res = tsfi_mf_nato_verify_up_ramp_rate_threshold(6, &up_ramp_rate_threshold_valid);
    assert(up_ramp_rate_threshold_res == 0);
    ...
    // 341. Verify NATO Link-Layer Broadcast Channel Busy Hysteresis Down-Ramp Rate Factor Matcher
    printf("[TEST] Validating NATO Broadcast Busy Down-Ramp Rate Factor Matcher...\n");
    int down_ramp_factor_valid = -1;
    int down_ramp_factor_res = tsfi_mf_nato_verify_down_ramp_rate_factor(4, &down_ramp_factor_valid);
    assert(down_ramp_factor_res == 0);
    assert(down_ramp_factor_valid == 1);
    printf("  [PASS] NATO Broadcast Busy Down-Ramp Rate Factor Matcher verified.\n");

    // 342. Verify NATO Link-Layer Broadcast Collision Backoff Slot Alignment Method Option Checker
    printf("[TEST] Validating NATO Broadcast Backoff Slot Method Option Checker...\n");
    int slot_method_option_valid = -1;
    int slot_method_option_res = tsfi_mf_nato_verify_slot_method_option(2, &slot_method_option_valid);
    assert(slot_method_option_res == 0);
    assert(slot_method_option_valid == 1);
    printf("  [PASS] NATO Broadcast Backoff Slot Method Option Checker verified.\n");

    // 343. Verify NATO Link-Layer Broadcast Collision Backoff Slot Alignment Boundary Option Matcher
    printf("[TEST] Validating NATO Broadcast Backoff Slot Boundary Option Checker...\n");
    int slot_boundary_option_valid = -1;
    int slot_boundary_option_res = tsfi_mf_nato_verify_slot_boundary_option(1, &slot_boundary_option_valid);
    assert(slot_boundary_option_res == 0);
    assert(slot_boundary_option_valid == 1);
    printf("  [PASS] NATO Broadcast Backoff Slot Boundary Option Checker verified.\n");

    // 344. Verify NATO Link-Layer Broadcast Channel Busy Hysteresis Up-Ramp Rate Limit Matcher
    printf("[TEST] Validating NATO Broadcast Busy Up-Ramp Rate Limit Matcher...\n");
    int up_ramp_limit_valid = -1;
    int up_ramp_limit_res = tsfi_mf_nato_verify_up_ramp_rate_limit(15, &up_ramp_limit_valid);
    assert(up_ramp_limit_res == 0);
    assert(up_ramp_limit_valid == 1);
    printf("  [PASS] NATO Broadcast Busy Up-Ramp Rate Limit Matcher verified.\n");

    // 345. Verify NATO Link-Layer Broadcast Channel Busy Hysteresis Down-Ramp Rate Limit Matcher
    printf("[TEST] Validating NATO Broadcast Busy Down-Ramp Rate Limit Matcher...\n");
    int down_ramp_limit_valid = -1;
    int down_ramp_limit_res = tsfi_mf_nato_verify_down_ramp_rate_limit(10, &down_ramp_limit_valid);
    assert(down_ramp_limit_res == 0);
    assert(down_ramp_limit_valid == 1);
    printf("  [PASS] NATO Broadcast Busy Down-Ramp Rate Limit Matcher verified.\n");

    // 346. Verify NATO Link-Layer Broadcast Collision Backoff Slot Alignment Offset Boundary Matcher
    printf("[TEST] Validating NATO Broadcast Backoff Slot Offset Boundary Matcher...\n");
    int slot_offset_boundary_valid = -1;
    int slot_offset_boundary_res = tsfi_mf_nato_verify_slot_offset_boundary(4, &slot_offset_boundary_valid);
    assert(slot_offset_boundary_res == 0);
    assert(slot_offset_boundary_valid == 1);
    printf("  [PASS] NATO Broadcast Backoff Slot Offset Boundary Matcher verified.\n");

    // 347. Verify NATO Link-Layer Broadcast Collision Backoff Slot Alignment Boundary Source Matcher
    printf("[TEST] Validating NATO Broadcast Backoff Slot Boundary Source Matcher...\n");
    int slot_boundary_source_valid = -1;
    int slot_boundary_source_res = tsfi_mf_nato_verify_slot_boundary_source(1, &slot_boundary_source_valid);
    assert(slot_boundary_source_res == 0);
    assert(slot_boundary_source_valid == 1);
    printf("  [PASS] NATO Broadcast Backoff Slot Boundary Source Matcher verified.\n");

    // 348. Verify NATO Link-Layer Broadcast Channel Busy Hysteresis Up-Ramp Rate Factor Offset Matcher
    printf("[TEST] Validating NATO Broadcast Busy Up-Ramp Rate Factor Offset Matcher...\n");
    int up_ramp_factor_offset_valid = -1;
    int up_ramp_factor_offset_res = tsfi_mf_nato_verify_up_ramp_rate_factor_offset(3, &up_ramp_factor_offset_valid);
    assert(up_ramp_factor_offset_res == 0);
    ...
    // 351. Verify NATO Link-Layer Broadcast Collision Backoff Slot Alignment Boundary Source Offset Matcher
    printf("[TEST] Validating NATO Broadcast Backoff Slot Boundary Source Offset Matcher...\n");
    int slot_boundary_source_offset_valid = -1;
    int slot_boundary_source_offset_res = tsfi_mf_nato_verify_slot_boundary_source_offset(6, &slot_boundary_source_offset_valid);
    assert(slot_boundary_source_offset_res == 0);
    assert(slot_boundary_source_offset_valid == 1);
    printf("  [PASS] NATO Broadcast Backoff Slot Boundary Source Offset Matcher verified.\n");

    // 352. Verify NATO Link-Layer Broadcast Channel Busy Hysteresis Up-Ramp Duration Threshold Matcher
    printf("[TEST] Validating NATO Broadcast Busy Up-Ramp Duration Threshold Matcher...\n");
    int up_ramp_duration_threshold_valid = -1;
    int up_ramp_duration_threshold_res = tsfi_mf_nato_verify_up_ramp_duration_threshold(5, &up_ramp_duration_threshold_valid);
    assert(up_ramp_duration_threshold_res == 0);
    assert(up_ramp_duration_threshold_valid == 1);
    printf("  [PASS] NATO Broadcast Busy Up-Ramp Duration Threshold Matcher verified.\n");

    // 353. Verify NATO Link-Layer Broadcast Channel Busy Hysteresis Down-Ramp Duration Threshold Matcher
    printf("[TEST] Validating NATO Broadcast Busy Down-Ramp Duration Threshold Matcher...\n");
    int down_ramp_duration_threshold_valid = -1;
    int down_ramp_duration_threshold_res = tsfi_mf_nato_verify_down_ramp_duration_threshold(8, &down_ramp_duration_threshold_valid);
    assert(down_ramp_duration_threshold_res == 0);
    assert(down_ramp_duration_threshold_valid == 1);
    printf("  [PASS] NATO Broadcast Busy Down-Ramp Duration Threshold Matcher verified.\n");

    // 354. Verify NATO Link-Layer Broadcast Collision Backoff Slot Alignment Boundary Source Offset Limit Matcher
    printf("[TEST] Validating NATO Broadcast Backoff Slot Boundary Source Offset Limit Matcher...\n");
    int slot_boundary_source_offset_limit_valid = -1;
    int slot_boundary_source_offset_limit_res = tsfi_mf_nato_verify_slot_boundary_source_offset_limit(5, &slot_boundary_source_offset_limit_valid);
    assert(slot_boundary_source_offset_limit_res == 0);
    ...
    // 357. Verify NATO Link-Layer Broadcast Collision Backoff Slot Alignment Boundary Source Offset Range Limit Matcher
    printf("[TEST] Validating NATO Broadcast Backoff Slot Boundary Source Offset Range Limit Matcher...\n");
    int slot_boundary_source_offset_range_limit_valid = -1;
    int slot_boundary_source_offset_range_limit_res = tsfi_mf_nato_verify_slot_boundary_source_offset_range_limit(15, &slot_boundary_source_offset_range_limit_valid);
    assert(slot_boundary_source_offset_range_limit_res == 0);
    assert(slot_boundary_source_offset_range_limit_valid == 1);
    printf("  [PASS] NATO Broadcast Backoff Slot Boundary Source Offset Range Limit Matcher verified.\n");

    // 358. Verify NATO Link-Layer Broadcast Channel Busy Hysteresis RSSI Quiet Sample Threshold Limit Matcher
    printf("[TEST] Validating NATO Broadcast Busy RSSI Quiet Sample Threshold Limit Matcher...\n");
    int rssi_quiet_sample_threshold_limit_valid = -1;
    int rssi_quiet_sample_threshold_limit_res = tsfi_mf_nato_verify_rssi_quiet_sample_threshold_limit(5, &rssi_quiet_sample_threshold_limit_valid);
    assert(rssi_quiet_sample_threshold_limit_res == 0);
    assert(rssi_quiet_sample_threshold_limit_valid == 1);
    printf("  [PASS] NATO Broadcast Busy RSSI Quiet Sample Threshold Limit Matcher verified.\n");

    // 359. Verify NATO Link-Layer Broadcast Collision Backoff Slot Alignment Boundary Source Offset Range Limit Boundary Matcher
    printf("[TEST] Validating NATO Broadcast Backoff Slot Boundary Source Offset Range Limit Boundary Matcher...\n");
    int slot_boundary_source_offset_range_limit_boundary_valid = -1;
    int slot_boundary_source_offset_range_limit_boundary_res = tsfi_mf_nato_verify_slot_boundary_source_offset_range_limit_boundary(20, &slot_boundary_source_offset_range_limit_boundary_valid);
    assert(slot_boundary_source_offset_range_limit_boundary_res == 0);
    assert(slot_boundary_source_offset_range_limit_boundary_valid == 1);
    printf("  [PASS] NATO Broadcast Backoff Slot Boundary Source Offset Range Limit Boundary Matcher verified.\n");

    // 360. Verify NATO Link-Layer Broadcast Channel Busy Hysteresis RSSI Quiet Sample Threshold Limit Margin Matcher
    printf("[TEST] Validating NATO Broadcast Busy RSSI Quiet Sample Threshold Limit Margin Matcher...\n");
    int rssi_quiet_sample_threshold_limit_margin_valid = -1;
    int rssi_quiet_sample_threshold_limit_margin_res = tsfi_mf_nato_verify_rssi_quiet_sample_threshold_limit_margin(3, &rssi_quiet_sample_threshold_limit_margin_valid);
    assert(rssi_quiet_sample_threshold_limit_margin_res == 0);
    assert(rssi_quiet_sample_threshold_limit_margin_valid == 1);
    printf("  [PASS] NATO Broadcast Busy RSSI Quiet Sample Threshold Limit Margin Matcher verified.\n");

    // 361. Verify NATO Link-Layer Broadcast Collision Backoff Slot Alignment Boundary Source Offset Range Limit Value Matcher
    printf("[TEST] Validating NATO Broadcast Backoff Slot Boundary Source Offset Range Limit Value Matcher...\n");
    int slot_boundary_source_offset_range_limit_val_valid = -1;
    int slot_boundary_source_offset_range_limit_val_res = tsfi_mf_nato_verify_slot_boundary_source_offset_range_limit_val(25, &slot_boundary_source_offset_range_limit_val_valid);
    assert(slot_boundary_source_offset_range_limit_val_res == 0);
    assert(slot_boundary_source_offset_range_limit_val_valid == 1);
    printf("  [PASS] NATO Broadcast Backoff Slot Boundary Source Offset Range Limit Value Matcher verified.\n");

    // 362. Verify NATO Link-Layer Broadcast Channel Busy Hysteresis RSSI Quiet Sample Threshold Limit Margin Range Matcher
    printf("[TEST] Validating NATO Broadcast Busy RSSI Quiet Sample Threshold Limit Margin Range Matcher...\n");
    int rssi_quiet_sample_threshold_limit_margin_range_valid = -1;
    int rssi_quiet_sample_threshold_limit_margin_range_res = tsfi_mf_nato_verify_rssi_quiet_sample_threshold_limit_margin_range(4, &rssi_quiet_sample_threshold_limit_margin_range_valid);
    assert(rssi_quiet_sample_threshold_limit_margin_range_res == 0);
    assert(rssi_quiet_sample_threshold_limit_margin_range_valid == 1);
    printf("  [PASS] NATO Broadcast Busy RSSI Quiet Sample Threshold Limit Margin Range Matcher verified.\n");

    // 363. Verify NATO Link-Layer Broadcast Collision Backoff Slot Alignment Boundary Source Offset Range Limit Value Boundary Matcher
    printf("[TEST] Validating NATO Broadcast Backoff Slot Boundary Source Offset Range Limit Value Boundary Matcher...\n");
    int slot_boundary_source_offset_range_limit_val_boundary_valid = -1;
    int slot_boundary_source_offset_range_limit_val_boundary_res = tsfi_mf_nato_verify_slot_boundary_source_offset_range_limit_val_boundary(30, &slot_boundary_source_offset_range_limit_val_boundary_valid);
    assert(slot_boundary_source_offset_range_limit_val_boundary_res == 0);
    assert(slot_boundary_source_offset_range_limit_val_boundary_valid == 1);
    printf("  [PASS] NATO Broadcast Backoff Slot Boundary Source Offset Range Limit Value Boundary Matcher verified.\n");

    // 364. Verify NATO Link-Layer Broadcast Channel Busy Hysteresis RSSI Quiet Sample Threshold Limit Margin Range Option Checker
    printf("[TEST] Validating NATO Broadcast Busy RSSI Quiet Sample Threshold Limit Margin Range Option Checker...\n");
    int rssi_quiet_sample_threshold_limit_margin_range_option_valid = -1;
    int rssi_quiet_sample_threshold_limit_margin_range_option_res = tsfi_mf_nato_verify_rssi_quiet_sample_threshold_limit_margin_range_option(2, &rssi_quiet_sample_threshold_limit_margin_range_option_valid);
    assert(rssi_quiet_sample_threshold_limit_margin_range_option_res == 0);
    assert(rssi_quiet_sample_threshold_limit_margin_range_option_valid == 1);
    printf("  [PASS] NATO Broadcast Busy RSSI Quiet Sample Threshold Limit Margin Range Option Checker verified.\n");

    // 365. Verify NATO Link-Layer Broadcast Collision Backoff Slot Alignment Boundary Source Offset Range Limit Value Margin Matcher
    printf("[TEST] Validating NATO Broadcast Backoff Slot Boundary Source Offset Range Limit Value Margin Matcher...\n");
    int slot_boundary_source_offset_range_limit_val_margin_valid = -1;
    int slot_boundary_source_offset_range_limit_val_margin_res = tsfi_mf_nato_verify_slot_boundary_source_offset_range_limit_val_margin(35, &slot_boundary_source_offset_range_limit_val_margin_valid);
    assert(slot_boundary_source_offset_range_limit_val_margin_res == 0);
    assert(slot_boundary_source_offset_range_limit_val_margin_valid == 1);
    printf("  [PASS] NATO Broadcast Backoff Slot Boundary Source Offset Range Limit Value Margin Matcher verified.\n");

    // 366. Verify NATO Link-Layer Broadcast Channel Busy Hysteresis RSSI Quiet Sample Threshold Limit Margin Range Value Matcher
    printf("[TEST] Validating NATO Broadcast Busy RSSI Quiet Sample Threshold Limit Margin Range Value Matcher...\n");
    int rssi_quiet_sample_threshold_limit_margin_range_val_valid = -1;
    int rssi_quiet_sample_threshold_limit_margin_range_val_res = tsfi_mf_nato_verify_rssi_quiet_sample_threshold_limit_margin_range_val(5, &rssi_quiet_sample_threshold_limit_margin_range_val_valid);
    assert(rssi_quiet_sample_threshold_limit_margin_range_val_res == 0);
    ...
    // 369. Verify NATO Link-Layer Broadcast Collision Backoff Slot Alignment Boundary Source Offset Range Limit Value Margin Range Option Selector Option Matcher
    printf("[TEST] Validating NATO Broadcast Backoff Slot Boundary Source Offset Range Limit Value Margin Range Option Selector Option Matcher...\n");
    int slot_boundary_source_offset_range_limit_val_margin_range_option_selector_option_valid = -1;
    int slot_boundary_source_offset_range_limit_val_margin_range_option_selector_option_res = tsfi_mf_nato_verify_slot_boundary_source_offset_range_limit_val_margin_range_option_selector_option(1, &slot_boundary_source_offset_range_limit_val_margin_range_option_selector_option_valid);
    assert(slot_boundary_source_offset_range_limit_val_margin_range_option_selector_option_res == 0);
    assert(slot_boundary_source_offset_range_limit_val_margin_range_option_selector_option_valid == 1);
    printf("  [PASS] NATO Broadcast Backoff Slot Boundary Source Offset Range Limit Value Margin Range Option Selector Option Matcher verified.\n");

    // 370. Verify NATO Link-Layer Broadcast Channel Busy Hysteresis RSSI Quiet Sample Threshold Limit Margin Range Option Selector Margin Matcher
    printf("[TEST] Validating NATO Broadcast Busy RSSI Quiet Sample Threshold Limit Margin Range Option Selector Margin Matcher...\n");
    int rssi_quiet_sample_threshold_limit_margin_range_option_selector_margin_valid = -1;
    int rssi_quiet_sample_threshold_limit_margin_range_option_selector_margin_res = tsfi_mf_nato_verify_rssi_quiet_sample_threshold_limit_margin_range_option_selector_margin(3, &rssi_quiet_sample_threshold_limit_margin_range_option_selector_margin_valid);
    assert(rssi_quiet_sample_threshold_limit_margin_range_option_selector_margin_res == 0);
    assert(rssi_quiet_sample_threshold_limit_margin_range_option_selector_margin_valid == 1);
    printf("  [PASS] NATO Broadcast Busy RSSI Quiet Sample Threshold Limit Margin Range Option Selector Margin Matcher verified.\n");

    // 371. Verify NATO Link-Layer Broadcast Collision Backoff Slot Alignment Boundary Source Offset Range Limit Value Margin Range Option Selector Option Margin Matcher
    printf("[TEST] Validating NATO Broadcast Backoff Slot Boundary Source Offset Range Limit Value Margin Range Option Selector Option Margin Matcher...\n");
    int slot_boundary_source_offset_range_limit_val_margin_range_option_selector_option_margin_valid = -1;
    int slot_boundary_source_offset_range_limit_val_margin_range_option_selector_option_margin_res = tsfi_mf_nato_verify_slot_boundary_source_offset_range_limit_val_margin_range_option_selector_option_margin(45, &slot_boundary_source_offset_range_limit_val_margin_range_option_selector_option_margin_valid);
    assert(slot_boundary_source_offset_range_limit_val_margin_range_option_selector_option_margin_res == 0);
    assert(slot_boundary_source_offset_range_limit_val_margin_range_option_selector_option_margin_valid == 1);
    printf("  [PASS] NATO Broadcast Backoff Slot Boundary Source Offset Range Limit Value Margin Range Option Selector Option Margin Matcher verified.\n");

    // 372. Verify NATO Link-Layer Broadcast Channel Busy Hysteresis RSSI Quiet Sample Threshold Limit Margin Range Option Selector Margin Range Matcher
    printf("[TEST] Validating NATO Broadcast Busy RSSI Quiet Sample Threshold Limit Margin Range Option Selector Margin Range Matcher...\n");
    int rssi_quiet_sample_threshold_limit_margin_range_option_selector_margin_range_valid = -1;
    int rssi_quiet_sample_threshold_limit_margin_range_option_selector_margin_range_res = tsfi_mf_nato_verify_rssi_quiet_sample_threshold_limit_margin_range_option_selector_margin_range(4, &rssi_quiet_sample_threshold_limit_margin_range_option_selector_margin_range_valid);
    assert(rssi_quiet_sample_threshold_limit_margin_range_option_selector_margin_range_res == 0);
    assert(rssi_quiet_sample_threshold_limit_margin_range_option_selector_margin_range_valid == 1);
    printf("  [PASS] NATO Broadcast Busy RSSI Quiet Sample Threshold Limit Margin Range Option Selector Margin Range Matcher verified.\n");

    // 373. Verify NATO Link-Layer Broadcast Collision Backoff Slot Alignment Boundary Source Offset Range Limit Value Margin Range Option Selector Option Margin Range Matcher
    printf("[TEST] Validating NATO Broadcast Backoff Slot Boundary Source Offset Range Limit Value Margin Range Option Selector Option Margin Range Matcher...\n");
    int slot_boundary_source_offset_range_limit_val_margin_range_option_selector_option_margin_range_valid = -1;
    int slot_boundary_source_offset_range_limit_val_margin_range_option_selector_option_margin_range_res = tsfi_mf_nato_verify_slot_boundary_source_offset_range_limit_val_margin_range_option_selector_option_margin_range(50, &slot_boundary_source_offset_range_limit_val_margin_range_option_selector_option_margin_range_valid);
    assert(slot_boundary_source_offset_range_limit_val_margin_range_option_selector_option_margin_range_res == 0);
    assert(slot_boundary_source_offset_range_limit_val_margin_range_option_selector_option_margin_range_valid == 1);
    printf("  [PASS] NATO Broadcast Backoff Slot Boundary Source Offset Range Limit Value Margin Range Option Selector Option Margin Range Matcher verified.\n");

    // 374. Verify NATO Link-Layer Broadcast Channel Busy Hysteresis RSSI Quiet Sample Threshold Limit Margin Range Option Selector Margin Range Option Matcher
    printf("[TEST] Validating NATO Broadcast Busy RSSI Quiet Sample Threshold Limit Margin Range Option Selector Margin Range Option Matcher...\n");
    int rssi_quiet_sample_threshold_limit_margin_range_option_selector_margin_range_option_valid = -1;
    int rssi_quiet_sample_threshold_limit_margin_range_option_selector_margin_range_option_res = tsfi_mf_nato_verify_rssi_quiet_sample_threshold_limit_margin_range_option_selector_margin_range_option(1, &rssi_quiet_sample_threshold_limit_margin_range_option_selector_margin_range_option_valid);
    assert(rssi_quiet_sample_threshold_limit_margin_range_option_selector_margin_range_option_res == 0);
    assert(rssi_quiet_sample_threshold_limit_margin_range_option_selector_margin_range_option_valid == 1);
    printf("  [PASS] NATO Broadcast Busy RSSI Quiet Sample Threshold Limit Margin Range Option Selector Margin Range Option Matcher verified.\n");

    // 375. Verify NATO Link-Layer Broadcast Collision Backoff Slot Alignment Boundary Source Offset Range Limit Value Margin Range Option Selector Option Margin Range Option Matcher
    printf("[TEST] Validating NATO Broadcast Backoff Slot Boundary Source Offset Range Limit Value Margin Range Option Selector Option Margin Range Option Matcher...\n");
    int slot_boundary_source_offset_range_limit_val_margin_range_option_selector_option_margin_range_option_valid = -1;
    int slot_boundary_source_offset_range_limit_val_margin_range_option_selector_option_margin_range_option_res = tsfi_mf_nato_verify_slot_boundary_source_offset_range_limit_val_margin_range_option_selector_option_margin_range_option(2, &slot_boundary_source_offset_range_limit_val_margin_range_option_selector_option_margin_range_option_valid);
    assert(slot_boundary_source_offset_range_limit_val_margin_range_option_selector_option_margin_range_option_res == 0);
    assert(slot_boundary_source_offset_range_limit_val_margin_range_option_selector_option_margin_range_option_valid == 1);
    printf("  [PASS] NATO Broadcast Backoff Slot Boundary Source Offset Range Limit Value Margin Range Option Selector Option Margin Range Option Matcher verified.\n");

    // 376. Verify NATO Link-Layer Broadcast Channel Busy Hysteresis RSSI Quiet Sample Threshold Limit Margin Range Option Selector Margin Range Option Selector Matcher
    printf("[TEST] Validating NATO Broadcast Busy RSSI Quiet Sample Threshold Limit Margin Range Option Selector Margin Range Option Selector Matcher...\n");
    int rssi_quiet_sample_threshold_limit_margin_range_option_selector_margin_range_option_selector_valid = -1;
    int rssi_quiet_sample_threshold_limit_margin_range_option_selector_margin_range_option_selector_res = tsfi_mf_nato_verify_rssi_quiet_sample_threshold_limit_margin_range_option_selector_margin_range_option_selector(3, &rssi_quiet_sample_threshold_limit_margin_range_option_selector_margin_range_option_selector_valid);
    assert(rssi_quiet_sample_threshold_limit_margin_range_option_selector_margin_range_option_selector_res == 0);
    assert(rssi_quiet_sample_threshold_limit_margin_range_option_selector_margin_range_option_selector_valid == 1);
    printf("  [PASS] NATO Broadcast Busy RSSI Quiet Sample Threshold Limit Margin Range Option Selector Margin Range Option Selector Matcher verified.\n");

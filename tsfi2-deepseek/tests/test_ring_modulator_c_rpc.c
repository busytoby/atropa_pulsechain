...
static void trigger_process_sample(const char *contract, const char *input_hex, const char *carrier_hex, char *out_val_hex, size_t out_max) {
    char padded_input[128];
    char padded_carrier[128];
    pad_left_zero(input_hex, padded_input);
    pad_left_zero(carrier_hex, padded_carrier);

    char payload[4096];
    // Function selector: 0x07a96d8c
    // Input 32 bytes + carrier 32 bytes
    snprintf(payload, sizeof(payload),
             "{\"jsonrpc\":\"2.0\",\"method\":\"eth_call\",\"params\":[{"
             "\"to\":\"%s\","
             "\"data\":\"0x07a96d8c%s%s\""
             "},\"latest\"],\"id\":1}",
             contract, padded_input, padded_carrier);
...
int main() {
    printf("=============================================================\n");
    printf("ELEKTOR ISSUE #5: RING MODULATOR MIXING TEST (ANVIL LIVE)\n");
    printf("=============================================================\n");

    printf("[C-Test] Compiling ringModulator Yul object...\n");
    char *mod_hex = malloc(131072);
    assert(mod_hex != NULL);
    compile_yul_to_bytecode("../solidity/bin/ringModulator.yul", mod_hex, 131072);

    char mod_addr[128];
    printf("[C-Test] Deploying RingModulator contract dynamically from C...\n");
    deploy_contract_from_c(mod_hex, mod_addr, sizeof(mod_addr));
    printf("  [DEPLOYED] RingModulator Address: %s\n", mod_addr);
    free(mod_hex);

    // Test 1: Signed fixed-point multiplication: 0.5 * 0.5 = 0.25
    // 0.5V -> 500000000000000000 -> 0x6f05b59d3b20000
    // Expected result: 0.25V -> 250000000000000000 -> 0x3782dace9d90000
    char out_val1[128];
    trigger_process_sample(mod_addr,
                           "6f05b59d3b20000",
                           "6f05b59d3b20000",
                           out_val1, sizeof(out_val1));
    printf("  [RESULT] Positive Modulated Output: %s\n", out_val1);

    // Test 2: Negative multiplication: -0.5 * 0.5 = -0.25
    // -0.5V -> -500000000000000000 -> 0xffffffffffffffffffffffffffffffffffffffffffffffffff90fa4a62c4e0000
    // Expected: -0.25V -> 0xffffffffffffffffffffffffffffffffffffffffffffffffffc87d25316270000
    char out_val2[128];
    trigger_process_sample(mod_addr,
                           "ffffffffffffffffffffffffffffffffffffffffffffffff90fa4a62c4e0000",
                           "6f05b59d3b20000",
                           out_val2, sizeof(out_val2));
    printf("  [RESULT] Negative Modulated Output: %s\n", out_val2);

    printf("[PASS] Ring modulator mixing logic verified successfully!\n");
    printf("=============================================================\n");
    return 0;
}

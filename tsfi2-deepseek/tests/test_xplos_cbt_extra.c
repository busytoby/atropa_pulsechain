...
int main(void) {
    ...
    // 162. Test JCL job resource limit modification logs history (cbtjcljoblimitstats)
    printf("[TEST] Testing JCL job resource limit logs history query...\n");
    bool jcljoblimitstats_ok = tsfi_xplos_shell_exec(&shell, &sched, "cbtjcljoblimitstats");
    assert(jcljoblimitstats_ok == true);
    tsfi_xplos_run(&sched);

    // 163. Test JCL job resource limit logs integrity status (cbtjcljoblimitstatsstatus)
    printf("[TEST] Testing JCL job resource limit logs integrity status query...\n");
    bool jcljoblimitstatsstatus_ok = tsfi_xplos_shell_exec(&shell, &sched, "cbtjcljoblimitstatsstatus");
    assert(jcljoblimitstatsstatus_ok == true);
    tsfi_xplos_run(&sched);

    // 164. Test JCL job resource limit logs integrity details (cbtjcljoblimitstatsstatusdetails)
    printf("[TEST] Testing JCL job resource limit logs integrity status details query...\n");
    bool jcljoblimitstatsstatusdetails_ok = tsfi_xplos_shell_exec(&shell, &sched, "cbtjcljoblimitstatsstatusdetails");
    ...
    // 168. Test JCL job resource limits reset logs history query (cbtjcljoblimitresetstats)
    printf("[TEST] Testing JCL job resource limits reset logs history query...\n");
    bool jcljoblimitresetstats_ok = tsfi_xplos_shell_exec(&shell, &sched, "cbtjcljoblimitresetstats");
    assert(jcljoblimitresetstats_ok == true);
    tsfi_xplos_run(&sched);

    // 169. Test JCL job resource limits reset logs integrity status query (cbtjcljoblimitresetstatsstatus)
    printf("[TEST] Testing JCL job resource limits reset logs integrity status query...\n");
    bool jcljoblimitresetstatsstatus_ok = tsfi_xplos_shell_exec(&shell, &sched, "cbtjcljoblimitresetstatsstatus");
    assert(jcljoblimitresetstatsstatus_ok == true);
    tsfi_xplos_run(&sched);

    // 170. Test JCL job resource limits reset logs integrity details query (cbtjcljoblimitresetstatsstatusdetails)
    printf("[TEST] Testing JCL job resource limits reset logs integrity details query...\n");
    bool jcljoblimitresetstatsstatusdetails_ok = tsfi_xplos_shell_exec(&shell, &sched, "cbtjcljoblimitresetstatsstatusdetails");
    assert(jcljoblimitresetstatsstatusdetails_ok == true);
    tsfi_xplos_run(&sched);

    // 171. Test JCL job limits reset logs audit progress details (cbtjcljoblimitresetstatsstatusdetailsprogress)
    printf("[TEST] Testing JCL job limits reset logs audit progress query...\n");
    bool jcljoblimitresetstatsstatusdetailsprogress_ok = tsfi_xplos_shell_exec(&shell, &sched, "cbtjcljoblimitresetstatsstatusdetailsprogress");
    assert(jcljoblimitresetstatsstatusdetailsprogress_ok == true);
    tsfi_xplos_run(&sched);

    // 172. Test JCL job verification checks (cbtjcljobchk)
    printf("[TEST] Testing JCL job verification check...\n");
    bool jcljobchk_ok = tsfi_xplos_shell_exec(&shell, &sched, "cbtjcljobchk JOB00162");
    assert(jcljobchk_ok == true);
    tsfi_xplos_run(&sched);

    // 173. Test JCL job verification checks stats (cbtjcljobchkstat)
    printf("[TEST] Testing JCL job verification stats query...\n");
    bool jcljobchkstat_ok = tsfi_xplos_shell_exec(&shell, &sched, "cbtjcljobchkstat");
    ...
    // 177. Test JCL job check logs audit progress details (cbtjcljobchkstatsstatusdetailsprogress)
    printf("[TEST] Testing JCL job check logs audit progress query...\n");
    bool jcljobchkstatsstatusdetailsprogress_ok = tsfi_xplos_shell_exec(&shell, &sched, "cbtjcljobchkstatsstatusdetailsprogress");
    assert(jcljobchkstatsstatusdetailsprogress_ok == true);
    tsfi_xplos_run(&sched);

    // 178. Test JCL job check verification reset (cbtjcljobchkreset)
    printf("[TEST] Testing JCL job verification checks reset...\n");
    bool jcljobchkreset_ok = tsfi_xplos_shell_exec(&shell, &sched, "cbtjcljobchkreset");
    assert(jcljobchkreset_ok == true);
    tsfi_xplos_run(&sched);

    // 179. Test JCL job check verification reset stats (cbtjcljobchkresetstat)
    printf("[TEST] Testing JCL job verification resets stats query...\n");
    bool jcljobchkresetstat_ok = tsfi_xplos_shell_exec(&shell, &sched, "cbtjcljobchkresetstat");
    assert(jcljobchkresetstat_ok == true);
    tsfi_xplos_run(&sched);

    // 180. Test JCL job verification checks reset logs history query (cbtjcljobchkresetstats)
    printf("[TEST] Testing JCL job verification checks reset logs history query...\n");
    bool jcljobchkresetstats_ok = tsfi_xplos_shell_exec(&shell, &sched, "cbtjcljobchkresetstats");
    assert(jcljobchkresetstats_ok == true);
    tsfi_xplos_run(&sched);

    // 181. Test JCL job verification checks reset logs integrity status query (cbtjcljobchkresetstatsstatus)
    printf("[TEST] Testing JCL job verification checks reset logs integrity status query...\n");
    bool jcljobchkresetstatsstatus_ok = tsfi_xplos_shell_exec(&shell, &sched, "cbtjcljobchkresetstatsstatus");
    assert(jcljobchkresetstatsstatus_ok == true);
    tsfi_xplos_run(&sched);

    // 182. Test JCL job verification checks reset logs integrity details query (cbtjcljobchkresetstatsstatusdetails)
    printf("[TEST] Testing JCL job verification checks reset logs integrity details query...\n");
    bool jcljobchkresetstatsstatusdetails_ok = tsfi_xplos_shell_exec(&shell, &sched, "cbtjcljobchkresetstatsstatusdetails");
    assert(jcljobchkresetstatsstatusdetails_ok == true);
    tsfi_xplos_run(&sched);

    // 183. Test JCL job verification checks reset logs audit progress details (cbtjcljobchkresetstatsstatusdetailsprogress)
    printf("[TEST] Testing JCL job verification checks reset logs audit progress query...\n");
    bool jcljobchkresetstatsstatusdetailsprogress_ok = tsfi_xplos_shell_exec(&shell, &sched, "cbtjcljobchkresetstatsstatusdetailsprogress");
    assert(jcljobchkresetstatsstatusdetailsprogress_ok == true);
    tsfi_xplos_run(&sched);

    // 184. Test JCL job statistics query (cbtjcljobstat)
    printf("[TEST] Testing JCL job statistics query...\n");
    bool jcljobstat_ok = tsfi_xplos_shell_exec(&shell, &sched, "cbtjcljobstat JOB00162");
    assert(jcljobstat_ok == true);
    tsfi_xplos_run(&sched);

    // 185. Test JCL job statistics query status (cbtjcljobstatstat)
    printf("[TEST] Testing JCL job statistics query status...\n");
    bool jcljobstatstat_ok = tsfi_xplos_shell_exec(&shell, &sched, "cbtjcljobstatstat");
    assert(jcljobstatstat_ok == true);
    tsfi_xplos_run(&sched);

    // 186. Test JCL job statistics logs history query (cbtjcljobstatstats)
    printf("[TEST] Testing JCL job statistics logs history query...\n");
    bool jcljobstatstats_ok = tsfi_xplos_shell_exec(&shell, &sched, "cbtjcljobstatstats");
    assert(jcljobstatstats_ok == true);
    tsfi_xplos_run(&sched);

    // 187. Test JCL job statistics logs integrity status query (cbtjcljobstatstatsstatus)
    printf("[TEST] Testing JCL job statistics logs integrity status query...\n");
    bool jcljobstatstatsstatus_ok = tsfi_xplos_shell_exec(&shell, &sched, "cbtjcljobstatstatsstatus");
    assert(jcljobstatstatsstatus_ok == true);
    tsfi_xplos_run(&sched);

    // 188. Test JCL job statistics logs integrity details query (cbtjcljobstatstatsstatusdetails)
    printf("[TEST] Testing JCL job statistics logs integrity details query...\n");
    bool jcljobstatstatsstatusdetails_ok = tsfi_xplos_shell_exec(&shell, &sched, "cbtjcljobstatstatsstatusdetails");
    assert(jcljobstatstatsstatusdetails_ok == true);
    tsfi_xplos_run(&sched);

    // 189. Test JCL job statistics logs audit progress details (cbtjcljobstatstatsstatusdetailsprogress)
    printf("[TEST] Testing JCL job statistics logs audit progress query...\n");
    bool jcljobstatstatsstatusdetailsprogress_ok = tsfi_xplos_shell_exec(&shell, &sched, "cbtjcljobstatstatsstatusdetailsprogress");
    assert(jcljobstatstatsstatusdetailsprogress_ok == true);
    tsfi_xplos_run(&sched);

    // 190. Test JCL job statistics reset (cbtjcljobstatreset)
    printf("[TEST] Testing JCL job statistics reset...\n");
    bool jcljobstatreset_ok = tsfi_xplos_shell_exec(&shell, &sched, "cbtjcljobstatreset");
    assert(jcljobstatreset_ok == true);
    tsfi_xplos_run(&sched);

    // 191. Test JCL job statistics reset stats (cbtjcljobstatresetstat)
    printf("[TEST] Testing JCL job statistics reset stats query...\n");
    bool jcljobstatresetstat_ok = tsfi_xplos_shell_exec(&shell, &sched, "cbtjcljobstatresetstat");
    assert(jcljobstatresetstat_ok == true);
    tsfi_xplos_run(&sched);

    // 192. Test JCL job statistics reset logs history query (cbtjcljobstatresetstats)
    printf("[TEST] Testing JCL job statistics reset logs history query...\n");
    bool jcljobstatresetstats_ok = tsfi_xplos_shell_exec(&shell, &sched, "cbtjcljobstatresetstats");
    assert(jcljobstatresetstats_ok == true);
    tsfi_xplos_run(&sched);

    // 193. Test JCL job statistics reset logs integrity status query (cbtjcljobstatresetstatsstatus)
    printf("[TEST] Testing JCL job statistics reset logs integrity status query...\n");
    bool jcljobstatresetstatsstatus_ok = tsfi_xplos_shell_exec(&shell, &sched, "cbtjcljobstatresetstatsstatus");
    assert(jcljobstatresetstatsstatus_ok == true);
    tsfi_xplos_run(&sched);

    // 194. Test JCL job statistics reset logs integrity details query (cbtjcljobstatresetstatsstatusdetails)
    printf("[TEST] Testing JCL job statistics reset logs integrity details query...\n");
    bool jcljobstatresetstatsstatusdetails_ok = tsfi_xplos_shell_exec(&shell, &sched, "cbtjcljobstatresetstatsstatusdetails");
    ...
    // 198. Test JCL job statistics threshold config logs history query (cbtjcljobstatlimitstats)
    printf("[TEST] Testing JCL job statistics limit logs history query...\n");
    bool jcljobstatlimitstats_ok = tsfi_xplos_shell_exec(&shell, &sched, "cbtjcljobstatlimitstats");
    assert(jcljobstatlimitstats_ok == true);
    tsfi_xplos_run(&sched);

    // 199. Test JCL job statistics threshold config logs integrity status query (cbtjcljobstatlimitstatsstatus)
    printf("[TEST] Testing JCL job statistics limit logs integrity status query...\n");
    bool jcljobstatlimitstatsstatus_ok = tsfi_xplos_shell_exec(&shell, &sched, "cbtjcljobstatlimitstatsstatus");
    assert(jcljobstatlimitstatsstatus_ok == true);
    tsfi_xplos_run(&sched);

    // 200. Test JCL job statistics threshold config logs integrity details query (cbtjcljobstatlimitstatsstatusdetails)
    printf("[TEST] Testing JCL job statistics limit logs integrity details query...\n");
    bool jcljobstatlimitstatsstatusdetails_ok = tsfi_xplos_shell_exec(&shell, &sched, "cbtjcljobstatlimitstatsstatusdetails");
    ...
    // 202. Test VTAM route recovery reset logs history query (vtamrouterecresetstats)
    printf("[TEST] Testing VTAM route recovery reset logs history query...\n");
    bool vtamrouterecresetstats_ok = tsfi_xplos_shell_exec(&shell, &sched, "vtamrouterecresetstats");
    assert(vtamrouterecresetstats_ok == true);
    tsfi_xplos_run(&sched);

    // 203. Test VTAM route recovery reset logs integrity status query (vtamrouterecresetstatsstatus)
    printf("[TEST] Testing VTAM route recovery reset logs integrity status query...\n");
    bool vtamrouterecresetstatsstatus_ok = tsfi_xplos_shell_exec(&shell, &sched, "vtamrouterecresetstatsstatus");
    assert(vtamrouterecresetstatsstatus_ok == true);
    tsfi_xplos_run(&sched);

    // 204. Test VTAM route recovery reset logs integrity details query (vtamrouterecresetstatsstatusdetails)
    printf("[TEST] Testing VTAM route recovery reset logs integrity details query...\n");
    bool vtamrouterecresetstatsstatusdetails_ok = tsfi_xplos_shell_exec(&shell, &sched, "vtamrouterecresetstatsstatusdetails");
    ...
    // 211. Test JES HASP spool logs history query (cbthaspspoollogs)
    printf("[TEST] Testing JES HASP spool logs history query...\n");
    bool cbthaspspoollogs_ok = tsfi_xplos_shell_exec(&shell, &sched, "cbthaspspoollogs");
    assert(cbthaspspoollogs_ok == true);
    tsfi_xplos_run(&sched);

    // 212. Test JES HASP spool logs integrity status query (cbthaspspoollogsstatus)
    printf("[TEST] Testing JES HASP spool logs integrity status query...\n");
    bool cbthaspspoollogsstatus_ok = tsfi_xplos_shell_exec(&shell, &sched, "cbthaspspoollogsstatus");
    assert(cbthaspspoollogsstatus_ok == true);
    tsfi_xplos_run(&sched);

    // 213. Test JES HASP spool logs integrity details query (cbthaspspoollogsstatusdetails)
    printf("[TEST] Testing JES HASP spool logs integrity details query...\n");
    bool cbthaspspoollogsstatusdetails_ok = tsfi_xplos_shell_exec(&shell, &sched, "cbthaspspoollogsstatusdetails");
    ...
    // 223. Test Book compliance progress query (cbtbookstatsstatusdetailsprogress)
    printf("[TEST] Testing Book compliance progress query...\n");
    bool cbtbookstatsstatusdetailsprogress_ok = tsfi_xplos_shell_exec(&shell, &sched, "cbtbookstatsstatusdetailsprogress");
    assert(cbtbookstatsstatusdetailsprogress_ok == true);
    tsfi_xplos_run(&sched);

    // 224. Test Book ignore list reset (cbtbookignorelistreset)
    printf("[TEST] Testing Book ignore list reset...\n");
    bool cbtbookignorelistreset_ok = tsfi_xplos_shell_exec(&shell, &sched, "cbtbookignorelistreset");
    assert(cbtbookignorelistreset_ok == true);
    tsfi_xplos_run(&sched);

    // 225. Test Book ignore list reset stats (cbtbookignorelistresetstat)
    printf("[TEST] Testing Book ignore list reset stats query...\n");
    bool cbtbookignorelistresetstat_ok = tsfi_xplos_shell_exec(&shell, &sched, "cbtbookignorelistresetstat");

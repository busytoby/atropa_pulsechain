...
int main(void) {
    ...
    // 148. Verify IRS IMF Miscellaneous Form Verifier
    printf("[TEST] Validating IRS IMF Miscellaneous Form Verifier...\n");
    int is_misc = -1;
    int misc_res = tsfi_mf_imf_is_misc_form(50, &is_misc);
    assert(misc_res == 0);
    assert(is_misc == 1);
    printf("  [PASS] IRS IMF Miscellaneous Form Verifier verified.\n");

    // 149. Verify IRS CADE Taxpayer Unknown Status Verifier
    printf("[TEST] Validating IRS CADE Taxpayer Unknown Status Verifier...\n");
    int is_unk = -1;
    int unk_status_res = tsfi_mf_cade_is_unknown_status(9, &is_unk);
    assert(unk_status_res == 0);
    assert(is_unk == 1);
    printf("  [PASS] IRS CADE Taxpayer Unknown Status Verifier verified.\n");

    // 150. Verify IRS IMF Partnership Form Verifier
    printf("[TEST] Validating IRS IMF Partnership Form Verifier...\n");
    int is_part = -1;
    int part_res = tsfi_mf_imf_is_partnership_form(15, &is_part);
    assert(part_res == 0);
    assert(is_part == 1);
    printf("  [PASS] IRS IMF Partnership Form Verifier verified.\n");

    // 151. Verify IRS CADE Taxpayer Dual Active Checker
    printf("[TEST] Validating IRS CADE Taxpayer Dual Active Checker...\n");
    int both_act = -1;
    int both_act_res = tsfi_mf_cade_are_both_active(1, 1, &both_act);
    assert(both_act_res == 0);
    assert(both_act == 1);
    printf("  [PASS] IRS CADE Taxpayer Dual Active Checker verified.\n");

    // 152. Verify IRS IMF Estate Form Verifier
    printf("[TEST] Validating IRS IMF Estate Form Verifier...\n");
    int is_est = -1;
    int est_res = tsfi_mf_imf_is_estate_form(25, &is_est);
    assert(est_res == 0);
    assert(is_est == 1);
    printf("  [PASS] IRS IMF Estate Form Verifier verified.\n");

    // 153. Verify IRS CADE Taxpayer Audit Or Pending Checker
    printf("[TEST] Validating IRS CADE Taxpayer Audit Or Pending Checker...\n");
    int is_aop = -1;
    int aop_res = tsfi_mf_cade_is_audit_or_pending(4, &is_aop);
    assert(aop_res == 0);
    assert(is_aop == 1);
    ...
    // 157. Verify IRS IMF Fiduciary Form Verifier
    printf("[TEST] Validating IRS IMF Fiduciary Form Verifier...\n");
    int is_fid = -1;
    int fid_res = tsfi_mf_imf_is_fiduciary_form(11, &is_fid);
    assert(fid_res == 0);
    assert(is_fid == 1);
    printf("  [PASS] IRS IMF Fiduciary Form Verifier verified.\n");

    // 158. Verify IRS CADE Taxpayer Active Or Under Audit Checker
    printf("[TEST] Validating IRS CADE Taxpayer Active Or Under Audit Checker...\n");
    int is_aua = -1;
    int aua_res = tsfi_mf_cade_is_active_or_under_audit(2, &is_aua);
    assert(aua_res == 0);
    assert(is_aua == 1);
    printf("  [PASS] IRS CADE Taxpayer Active Or Under Audit Checker verified.\n");

    // 159. Verify IRS IMF Form 706 Verifier
    printf("[TEST] Validating IRS IMF Form 706 Verifier...\n");
    int is_706 = -1;
    int f706_res = tsfi_mf_imf_is_form_706(27, &is_706);
    assert(f706_res == 0);
    assert(is_706 == 1);
    printf("  [PASS] IRS IMF Form 706 Verifier verified.\n");

    // 160. Verify IRS CADE Taxpayer Active Or Audit Pending Checker
    printf("[TEST] Validating IRS CADE Taxpayer Active Or Audit Pending Checker...\n");
    int is_aap = -1;
    int aap_res = tsfi_mf_cade_is_active_or_audit_pending(4, &is_aap);
    assert(aap_res == 0);
    assert(is_aap == 1);
    printf("  [PASS] IRS CADE Taxpayer Active Or Audit Pending Checker verified.\n");

    // 161. Verify IRS IMF Form 709 Verifier
    printf("[TEST] Validating IRS IMF Form 709 Verifier...\n");
    int is_709 = -1;
    int f709_res = tsfi_mf_imf_is_form_709(28, &is_709);
    assert(f709_res == 0);
    assert(is_709 == 1);
    printf("  [PASS] IRS IMF Form 709 Verifier verified.\n");

    // 162. Verify IRS CADE Taxpayer Under Audit Or Suspended Checker
    printf("[TEST] Validating IRS CADE Taxpayer Under Audit Or Suspended Checker...\n");
    int is_uas = -1;
    int uas_res = tsfi_mf_cade_is_under_audit_or_suspended(3, &is_uas);
    assert(uas_res == 0);
    assert(is_uas == 1);
    printf("  [PASS] IRS CADE Taxpayer Under Audit Or Suspended Checker verified.\n");

    // 163. Verify IRS IMF Form 1041 Verifier
    printf("[TEST] Validating IRS IMF Form 1041 Verifier...\n");
    int is_1041 = -1;
    int f1041_res = tsfi_mf_imf_is_form_1041(29, &is_1041);
    assert(f1041_res == 0);
    assert(is_1041 == 1);
    printf("  [PASS] IRS IMF Form 1041 Verifier verified.\n");

    // 164. Verify IRS CADE Taxpayer Under Audit Or Audit Pending Checker
    printf("[TEST] Validating IRS CADE Taxpayer Under Audit Or Audit Pending Checker...\n");
    int is_uap = -1;
    int uap_res = tsfi_mf_cade_is_under_audit_or_pending(4, &is_uap);
    assert(uap_res == 0);
    assert(is_uap == 1);
    printf("  [PASS] IRS CADE Taxpayer Under Audit Or Audit Pending Checker verified.\n");

    // 165. Verify IRS IMF Form 1065 Verifier
    printf("[TEST] Validating IRS IMF Form 1065 Verifier...\n");
    int is_1065 = -1;
    int f1065_res = tsfi_mf_imf_is_form_1065(31, &is_1065);
    assert(f1065_res == 0);
    assert(is_1065 == 1);
    printf("  [PASS] IRS IMF Form 1065 Verifier verified.\n");

    // 166. Verify IRS CADE Taxpayer Active Or Under Audit Or Pending Checker
    printf("[TEST] Validating IRS CADE Taxpayer Active Or Under Audit Or Pending Checker...\n");
    int is_auap = -1;
    int auap_res = tsfi_mf_cade_is_active_or_under_audit_or_pending(4, &is_auap);
    assert(auap_res == 0);
    assert(is_auap == 1);
    printf("  [PASS] IRS CADE Taxpayer Active Or Under Audit Or Pending Checker verified.\n");

    // 167. Verify IRS IMF Form 1040 Verifier
    printf("[TEST] Validating IRS IMF Form 1040 Verifier...\n");
    int is_1040 = -1;
    int f1040_res = tsfi_mf_imf_is_form_1040(10, &is_1040);
    assert(f1040_res == 0);
    assert(is_1040 == 1);
    printf("  [PASS] IRS IMF Form 1040 Verifier verified.\n");

    // 168. Verify IRS CADE Taxpayer Active Or Suspended Or Pending Checker
    printf("[TEST] Validating IRS CADE Taxpayer Active Or Suspended Or Pending Checker...\n");
    int is_asop = -1;
    int asop_res = tsfi_mf_cade_is_active_or_suspended_or_pending(4, &is_asop);
    assert(asop_res == 0);
    assert(is_asop == 1);
    printf("  [PASS] IRS CADE Taxpayer Active Or Suspended Or Pending Checker verified.\n");

    // 169. Verify IRS IMF Form 941 Verifier
    printf("[TEST] Validating IRS IMF Form 941 Verifier...\n");
    int is_941 = -1;
    int f941_res = tsfi_mf_imf_is_form_941(30, &is_941);
    assert(f941_res == 0);
    assert(is_941 == 1);
    printf("  [PASS] IRS IMF Form 941 Verifier verified.\n");

    // 170. Verify IRS CADE Taxpayer Under Audit Or Suspended Or Pending Checker
    printf("[TEST] Validating IRS CADE Taxpayer Under Audit Or Suspended Or Pending Checker...\n");
    int is_uasp = -1;
    int uasp_res = tsfi_mf_cade_is_under_audit_or_suspended_or_pending(4, &is_uasp);
    assert(uasp_res == 0);
    assert(is_uasp == 1);
    ...
    // 177. Verify IRS IMF Form 1040EZ Verifier
    printf("[TEST] Validating IRS IMF Form 1040EZ Verifier...\n");
    int is_1040ez = -1;
    int f1040ez_res = tsfi_mf_imf_is_form_1040ez(34, &is_1040ez);
    assert(f1040ez_res == 0);
    assert(is_1040ez == 1);
    printf("  [PASS] IRS IMF Form 1040EZ Verifier verified.\n");

    // 178. Verify IRS CADE Taxpayer Under Audit Or Suspended Or Pending Checker Ver2
    printf("[TEST] Validating IRS CADE Taxpayer Under Audit Or Suspended Or Pending Checker Ver2...\n");
    int is_uasp2 = -1;
    int uasp2_res = tsfi_mf_cade_is_under_audit_or_suspended_or_pending_ver2(4, &is_uasp2);
    assert(uasp2_res == 0);
    assert(is_uasp2 == 1);
    ...
    // 181. Verify IRS IMF Form 1040-SR Verifier
    printf("[TEST] Validating IRS IMF Form 1040-SR Verifier...\n");
    int is_1040sr = -1;
    int f1040sr_res = tsfi_mf_imf_is_form_1040sr(36, &is_1040sr);
    assert(f1040sr_res == 0);
    assert(is_1040sr == 1);
    printf("  [PASS] IRS IMF Form 1040-SR Verifier verified.\n");

    // 182. Verify IRS CADE Taxpayer Under Audit Or Suspended Checker Ver3
    printf("[TEST] Validating IRS CADE Taxpayer Under Audit Or Suspended Checker Ver3...\n");
    int is_uas3 = -1;
    int uas3_res = tsfi_mf_cade_is_under_audit_or_suspended_ver3(3, &is_uas3);
    assert(uas3_res == 0);
    assert(is_uas3 == 1);
    printf("  [PASS] IRS CADE Taxpayer Under Audit Or Suspended Checker Ver3 verified.\n");

    // 183. Verify IRS IMF Form 1040-NR Verifier
    printf("[TEST] Validating IRS IMF Form 1040-NR Verifier...\n");
    int is_1040nr = -1;
    int f1040nr_res = tsfi_mf_imf_is_form_1040nr(37, &is_1040nr);
    assert(f1040nr_res == 0);
    assert(is_1040nr == 1);
    printf("  [PASS] IRS IMF Form 1040-NR Verifier verified.\n");

    // 184. Verify IRS CADE Taxpayer Active Or Under Audit Or Suspended Or Pending Checker Ver2
    printf("[TEST] Validating IRS CADE Taxpayer Active Or Under Audit Or Suspended Or Pending Checker Ver2...\n");
    int is_auasp2 = -1;
    int auasp2_res = tsfi_mf_cade_is_active_or_under_audit_or_suspended_or_pending_ver2(4, &is_auasp2);
    assert(auasp2_res == 0);
    assert(is_auasp2 == 1);
    printf("  [PASS] IRS CADE Taxpayer Active Or Under Audit Or Suspended Or Pending Checker Ver2 verified.\n");

    // 185. Verify IRS IMF Form 1040-PR Verifier
    printf("[TEST] Validating IRS IMF Form 1040-PR Verifier...\n");
    int is_1040pr = -1;
    int f1040pr_res = tsfi_mf_imf_is_form_1040pr(38, &is_1040pr);
    assert(f1040pr_res == 0);
    assert(is_1040pr == 1);
    printf("  [PASS] IRS IMF Form 1040-PR Verifier verified.\n");

    // 186. Verify IRS CADE Taxpayer Active Or Under Audit Or Suspended Or Pending Checker Ver3
    printf("[TEST] Validating IRS CADE Taxpayer Active Or Under Audit Or Suspended Or Pending Checker Ver3...\n");
    int is_auasp3 = -1;
    int auasp3_res = tsfi_mf_cade_is_active_or_under_audit_or_suspended_or_pending_ver3(4, &is_auasp3);
    assert(auasp3_res == 0);
    assert(is_auasp3 == 1);
    printf("  [PASS] IRS CADE Taxpayer Active Or Under Audit Or Suspended Or Pending Checker Ver3 verified.\n");

    // 187. Verify IRS IMF Form 1040-SS Verifier
    printf("[TEST] Validating IRS IMF Form 1040-SS Verifier...\n");
    int is_1040ss = -1;
    int f1040ss_res = tsfi_mf_imf_is_form_1040ss(39, &is_1040ss);
    assert(f1040ss_res == 0);
    assert(is_1040ss == 1);
    printf("  [PASS] IRS IMF Form 1040-SS Verifier verified.\n");

    // 188. Verify IRS CADE Taxpayer Active Or Under Audit Or Suspended Or Pending Checker Ver4
    printf("[TEST] Validating IRS CADE Taxpayer Active Or Under Audit Or Suspended Or Pending Checker Ver4...\n");
    int is_auasp4 = -1;
    int auasp4_res = tsfi_mf_cade_is_active_or_under_audit_or_suspended_or_pending_ver4(4, &is_auasp4);
    assert(auasp4_res == 0);
    assert(is_auasp4 == 1);
    printf("  [PASS] IRS CADE Taxpayer Active Or Under Audit Or Suspended Or Pending Checker Ver4 verified.\n");

    // 189. Verify IRS IMF Form 1040-C Verifier
    printf("[TEST] Validating IRS IMF Form 1040-C Verifier...\n");
    int is_1040c = -1;
    int f1040c_res = tsfi_mf_imf_is_form_1040c(40, &is_1040c);
    assert(f1040c_res == 0);
    assert(is_1040c == 1);
    printf("  [PASS] IRS IMF Form 1040-C Verifier verified.\n");

    // 190. Verify IRS CADE Taxpayer Active Or Under Audit Or Suspended Or Pending Checker Ver5
    printf("[TEST] Validating IRS CADE Taxpayer Active Or Under Audit Or Suspended Or Pending Checker Ver5...\n");
    int is_auasp5 = -1;
    int auasp5_res = tsfi_mf_cade_is_active_or_under_audit_or_suspended_or_pending_ver5(4, &is_auasp5);
    assert(auasp5_res == 0);
    assert(is_auasp5 == 1);
    printf("  [PASS] IRS CADE Taxpayer Active Or Under Audit Or Suspended Or Pending Checker Ver5 verified.\n");

    // 191. Verify IRS IMF Form 1040-NR-EZ Verifier
    printf("[TEST] Validating IRS IMF Form 1040-NR-EZ Verifier...\n");
    int is_1040nrez = -1;
    int f1040nrez_res = tsfi_mf_imf_is_form_1040nrez(41, &is_1040nrez);
    assert(f1040nrez_res == 0);
    assert(is_1040nrez == 1);
    printf("  [PASS] IRS IMF Form 1040-NR-EZ Verifier verified.\n");

    // 192. Verify IRS CADE Taxpayer Active Or Under Audit Or Suspended Or Pending Checker Ver6
    printf("[TEST] Validating IRS CADE Taxpayer Active Or Under Audit Or Suspended Or Pending Checker Ver6...\n");
    int is_auasp6 = -1;
    int auasp6_res = tsfi_mf_cade_is_active_or_under_audit_or_suspended_or_pending_ver6(4, &is_auasp6);
    assert(auasp6_res == 0);
    assert(is_auasp6 == 1);
    printf("  [PASS] IRS CADE Taxpayer Active Or Under Audit Or Suspended Or Pending Checker Ver6 verified.\n");

    // 193. Verify IRS IMF Form 1040-PR-EZ Verifier
    printf("[TEST] Validating IRS IMF Form 1040-PR-EZ Verifier...\n");
    int is_1040prez = -1;
    int f1040prez_res = tsfi_mf_imf_is_form_1040prez(42, &is_1040prez);
    assert(f1040prez_res == 0);
    assert(is_1040prez == 1);
    printf("  [PASS] IRS IMF Form 1040-PR-EZ Verifier verified.\n");

    // 194. Verify IRS CADE Taxpayer Active Or Under Audit Or Suspended Or Pending Checker Ver7
    printf("[TEST] Validating IRS CADE Taxpayer Active Or Under Audit Or Suspended Or Pending Checker Ver7...\n");
    int is_auasp7 = -1;
    int auasp7_res = tsfi_mf_cade_is_active_or_under_audit_or_suspended_or_pending_ver7(4, &is_auasp7);
    assert(auasp7_res == 0);
    assert(is_auasp7 == 1);
    printf("  [PASS] IRS CADE Taxpayer Active Or Under Audit Or Suspended Or Pending Checker Ver7 verified.\n");

    // 195. Verify IRS IMF Form 1040-SS-EZ Verifier
    printf("[TEST] Validating IRS IMF Form 1040-SS-EZ Verifier...\n");
    int is_1040ssez = -1;
    int f1040ssez_res = tsfi_mf_imf_is_form_1040ssez(43, &is_1040ssez);
    assert(f1040ssez_res == 0);
    assert(is_1040ssez == 1);
    printf("  [PASS] IRS IMF Form 1040-SS-EZ Verifier verified.\n");

    // 196. Verify IRS CADE Taxpayer Active Or Under Audit Or Suspended Or Pending Checker Ver8
    printf("[TEST] Validating IRS CADE Taxpayer Active Or Under Audit Or Suspended Or Pending Checker Ver8...\n");
    int is_auasp8 = -1;
    int auasp8_res = tsfi_mf_cade_is_active_or_under_audit_or_suspended_or_pending_ver8(4, &is_auasp8);
    assert(auasp8_res == 0);
    assert(is_auasp8 == 1);
    printf("  [PASS] IRS CADE Taxpayer Active Or Under Audit Or Suspended Or Pending Checker Ver8 verified.\n");

    // 197. Verify IRS IMF Form 1040-PR-SS Verifier
    printf("[TEST] Validating IRS IMF Form 1040-PR-SS Verifier...\n");
    int is_1040prss = -1;
    int f1040prss_res = tsfi_mf_imf_is_form_1040prss(44, &is_1040prss);
    assert(f1040prss_res == 0);
    assert(is_1040prss == 1);
    printf("  [PASS] IRS IMF Form 1040-PR-SS Verifier verified.\n");

    // 198. Verify IRS CADE Taxpayer Active Or Under Audit Or Suspended Or Pending Checker Ver9
    printf("[TEST] Validating IRS CADE Taxpayer Active Or Under Audit Or Suspended Or Pending Checker Ver9...\n");
    int is_auasp9 = -1;
    int auasp9_res = tsfi_mf_cade_is_active_or_under_audit_or_suspended_or_pending_ver9(4, &is_auasp9);
    assert(auasp9_res == 0);
    assert(is_auasp9 == 1);
    printf("  [PASS] IRS CADE Taxpayer Active Or Under Audit Or Suspended Or Pending Checker Ver9 verified.\n");

    // 199. Verify IRS IMF Form 1040-SS-SS Verifier
    printf("[TEST] Validating IRS IMF Form 1040-SS-SS Verifier...\n");
    int is_1040ssss = -1;
    int f1040ssss_res = tsfi_mf_imf_is_form_1040ssss(45, &is_1040ssss);
    assert(f1040ssss_res == 0);
    assert(is_1040ssss == 1);
    printf("  [PASS] IRS IMF Form 1040-SS-SS Verifier verified.\n");

    // 200. Verify IRS CADE Taxpayer Active Or Under Audit Or Suspended Or Pending Checker Ver10
    printf("[TEST] Validating IRS CADE Taxpayer Active Or Under Audit Or Suspended Or Pending Checker Ver10...\n");
    int is_auasp10 = -1;
    int auasp10_res = tsfi_mf_cade_is_active_or_under_audit_or_suspended_or_pending_ver10(4, &is_auasp10);
    assert(auasp10_res == 0);
    assert(is_auasp10 == 1);
    printf("  [PASS] IRS CADE Taxpayer Active Or Under Audit Or Suspended Or Pending Checker Ver10 verified.\n");

    // 201. Verify IRS IMF Form 1040-SS-SS Ver2 Verifier
    printf("[TEST] Validating IRS IMF Form 1040-SS-SS Ver2 Verifier...\n");
    int is_1040ssss_ver2 = -1;
    int f1040ssss_ver2_res = tsfi_mf_imf_is_form_1040ssss_ver2(46, &is_1040ssss_ver2);
    assert(f1040ssss_ver2_res == 0);
    assert(is_1040ssss_ver2 == 1);
    printf("  [PASS] IRS IMF Form 1040-SS-SS Ver2 Verifier verified.\n");

    // 202. Verify IRS CADE Taxpayer Active Or Under Audit Or Suspended Or Pending Checker Ver11
    printf("[TEST] Validating IRS CADE Taxpayer Active Or Under Audit Or Suspended Or Pending Checker Ver11...\n");
    int is_auasp11 = -1;
    int auasp11_res = tsfi_mf_cade_is_active_or_under_audit_or_suspended_or_pending_ver11(4, &is_auasp11);
    assert(auasp11_res == 0);
    assert(is_auasp11 == 1);
    printf("  [PASS] IRS CADE Taxpayer Active Or Under Audit Or Suspended Or Pending Checker Ver11 verified.\n");

    // 203. Verify IRS IMF Form 1040-SS-SS Ver3 Verifier
    printf("[TEST] Validating IRS IMF Form 1040-SS-SS Ver3 Verifier...\n");
    int is_1040ssss_ver3 = -1;
    int f1040ssss_ver3_res = tsfi_mf_imf_is_form_1040ssss_ver3(47, &is_1040ssss_ver3);
    assert(f1040ssss_ver3_res == 0);
    assert(is_1040ssss_ver3 == 1);
    printf("  [PASS] IRS IMF Form 1040-SS-SS Ver3 Verifier verified.\n");

    // 204. Verify IRS CADE Taxpayer Active Or Under Audit Or Suspended Or Pending Checker Ver12
    printf("[TEST] Validating IRS CADE Taxpayer Active Or Under Audit Or Suspended Or Pending Checker Ver12...\n");
    int is_auasp12 = -1;
    int auasp12_res = tsfi_mf_cade_is_active_or_under_audit_or_suspended_or_pending_ver12(4, &is_auasp12);
    assert(auasp12_res == 0);
    assert(is_auasp12 == 1);
    printf("  [PASS] IRS CADE Taxpayer Active Or Under Audit Or Suspended Or Pending Checker Ver12 verified.\n");

    // 205. Verify IRS IMF Form 1040-SS-SS Ver4 Verifier
    printf("[TEST] Validating IRS IMF Form 1040-SS-SS Ver4 Verifier...\n");
    int is_1040ssss_ver4 = -1;
    int f1040ssss_ver4_res = tsfi_mf_imf_is_form_1040ssss_ver4(48, &is_1040ssss_ver4);
    assert(f1040ssss_ver4_res == 0);
    assert(is_1040ssss_ver4 == 1);
    printf("  [PASS] IRS IMF Form 1040-SS-SS Ver4 Verifier verified.\n");

    // 206. Verify IRS CADE Taxpayer Active Or Under Audit Or Suspended Or Pending Checker Ver13
    printf("[TEST] Validating IRS CADE Taxpayer Active Or Under Audit Or Suspended Or Pending Checker Ver13...\n");
    int is_auasp13 = -1;
    int auasp13_res = tsfi_mf_cade_is_active_or_under_audit_or_suspended_or_pending_ver13(4, &is_auasp13);
    assert(auasp13_res == 0);
    assert(is_auasp13 == 1);
    printf("  [PASS] IRS CADE Taxpayer Active Or Under Audit Or Suspended Or Pending Checker Ver13 verified.\n");

...
struct YI* tsfi_reaction_fuse(TSFiBigInt* Xi, TSFiBigInt* Rho, TSFiBigInt* Ohm, TSFiBigInt* Prime) {
...
int main() {
    ...
    TSFiBigInt* Pi = orig->Ring ? orig->Ring : orig->Xi;
    struct Dai* orig_dai = tsfi_reaction_compute_reciprocity(orig->Psi, Pi, NULL);

    struct YI* fused = tsfi_reaction_fuse(Pi, orig_dai->Daiichi, orig_dai->Ichidai, Prime);
    TSFiBigInt* fused_Pi = fused->Ring ? fused->Ring : fused->Xi;

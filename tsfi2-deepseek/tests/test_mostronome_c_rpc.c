...
static void trigger_process_samples(const char *contract, const char *count_hex, char *out_val_hex, size_t out_max) {
...
int main() {
    printf("=============================================================\n");
    printf("ELEKTOR ISSUE #3: MOSTRONOME TICK GENERATOR TEST (ANVIL LIVE)\n");
    printf("=============================================================\n");

    printf("[C-Test] Compiling Mostronome Yul object...\n");
    char *most_hex = malloc(131072);
    assert(most_hex != NULL);
    compile_yul_to_bytecode("../solidity/bin/mostronome.yul", most_hex, 131072);

    char most_addr[128];
    printf("[C-Test] Deploying Mostronome contract dynamically from C...\n");
    deploy_contract_from_c(most_hex, most_addr, sizeof(most_addr));
    printf("  [DEPLOYED] Mostronome Address: %s\n", most_addr);
    free(most_hex);

    // Test 1: Query a batch of 4 metronome samples to check the wave decay envelope
    char out_val[1024];
    trigger_process_samples(most_addr, "4", out_val, sizeof(out_val));
    printf("  [RESULT] Metronome Sine Wave Decay Samples (0-3):\n");
    printf("    Sample 0: %.64s\n", out_val + 2);
    printf("    Sample 1: %.64s\n", out_val + 2 + 64);
    printf("    Sample 2: %.64s\n", out_val + 2 + 128);
    printf("    Sample 3: %.64s\n", out_val + 2 + 192);

    printf("[PASS] Mostronome electronic metronome verified successfully!\n");
    printf("=============================================================\n");
    return 0;
}

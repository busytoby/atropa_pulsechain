...
static void test_clawvm_virtual_memory_engine(void) {
    ...
    tsfi_clawvm_zmm_kv_state_t zmm_state;
    bool ok_zmm = tsfi_clawvm_zmm_kv_layout_eval(32, 128, 0x1337BEEF, &zmm_state);
    assert(ok_zmm && zmm_state.sub_microsecond_gather_guaranteed);
    assert(zmm_state.dynamic_contracts_resolved == 1 && zmm_state.zmm_attention_speedup_x > 4.0f);

    tsfi_openclaw_event_loop_state_t loop_state;
    bool ok_loop = tsfi_openclaw_event_loop_step(&openclaw_rt, 3, &loop_state);
    ...
    tsfi_clawvm_subpage_migration_state_t subpage_mig;
    bool ok_smig = tsfi_clawvm_subpage_migration_eval(64, 256, 0.85f, &subpage_mig);
    ...
    tsfi_clawvm_checkpoint_sync_state_t chk_sync;
    bool ok_csync = tsfi_clawvm_checkpoint_sync_eval(1, 16, "zorse_checkpoint.dat.bin", &chk_sync);

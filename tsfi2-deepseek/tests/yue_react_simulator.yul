object "YueReactSimulator" {
    ...
    object "runtime" {
        code {
            ...
            // Selector signature: 0xcb1b70ff
            if eq(selector, 0xcb1b70ff) {
                let qing := calldataload(4)
                let charge := 125
                mstore(0, charge)
                return(0, 32)
            ...
            // Selector signature: 0x4858e8b2
            if eq(selector, 0x4858e8b2) {
                let qing := calldataload(4)
                let charge := 125
                let omicron := 25000000000
                let omega := 77700000000
                mstore(0, charge)
                mstore(32, omicron)
                mstore(64, omega)
                return(0, 96)
            ...
            // Spin(uint256 QingWaat) -> returns (uint256 Iota, uint256 Omicron, uint256 Omega, uint256 Eta)
            // Selector signature: 0xdc0042a9
            if eq(selector, 0xdc0042a9) {
                let qing := calldataload(4)
                let iota := 12000
                let omicron := 8877
                let omega := 4433

...
int main(void) {
    ...
    // 2. Direct Network Assertions
    assert(status_code == 403);
    assert(has_turnstile_marker == true);
    assert(has_binary_facsimile == false);
    printf("2. Live Endpoint Interception Verified: HTTP %d (Cloudflare Barrier)\n", status_code);

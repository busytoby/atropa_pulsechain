...
int main(void) {
    ...
    tsfi_agent_vm_operator_t teddy_op;
    ...
    assert(tsfi_agent_vm_operator_init(&teddy_op, 101, "TeddyBear_Operator", &p_vec));
    ...
    assert(tsfi_agent_vm_operator_mount_skill(&teddy_op, &catalog, "scsi_zmm_handshake", &fw));
    assert(teddy_op.mounted_skills_count == 1);

    char exec_buf[512] = {0};
    assert(tsfi_agent_vm_operator_execute(&teddy_op, &catalog, "scsi_zmm_handshake", "reg=0x32 keycode=d", exec_buf, sizeof(exec_buf)));

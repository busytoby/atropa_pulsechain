...
int main() {
    printf("=============================================================\n");
    printf("ELEKTOR ISSUE #18: OPTICAL VACTROL LIMITER (ANVIL LIVE)\n");
    printf("=============================================================\n");

    printf("[C-Test] Compiling VactrolLimiter Yul object...\n");
    char *vac_hex = malloc(131072);
    assert(vac_hex != NULL);
    compile_yul_to_bytecode("../solidity/bin/vactrolLimiter.yul", vac_hex, 131072);

    char vac_addr[128];
    printf("[C-Test] Deploying VactrolLimiter contract dynamically from C...\n");
    deploy_contract_from_c(vac_hex, vac_addr, sizeof(vac_addr));

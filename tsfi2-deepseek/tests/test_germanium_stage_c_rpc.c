...
int main() {
    printf("=============================================================\n");
    printf("ELEKTOR ISSUE #2/1: GERMANIUM TRANSISTOR BIAS TEST (ANVIL LIVE)\n");
    printf("=============================================================\n");

    printf("[C-Test] Compiling germaniumStage Yul object...\n");
    char *ger_hex = malloc(131072);
    assert(ger_hex != NULL);
    compile_yul_to_bytecode("../solidity/bin/germaniumStage.yul", ger_hex, 131072);

    char ger_addr[128];
    printf("[C-Test] Deploying GermaniumStage contract dynamically from C...\n");
    deploy_contract_from_c(ger_hex, ger_addr, sizeof(ger_addr));
    printf("  [DEPLOYED] GermaniumStage Address: %s\n", ger_addr);
    free(ger_hex);

    // Test 1: Below forward bias voltage (0.05V input)
    // 0.05V -> 50000000000000000 -> 0xb1a2bc2ec50000
    // Expected output: approx Vcc (9.0V) -> 9000000000000000000 -> 0x7ce66c50e2840000
    char out_val1[128];
    trigger_process_sample(ger_addr, "b1a2bc2ec50000", "0", out_val1, sizeof(out_val1));
    printf("  [RESULT] Idle Base Input Voltage: %s\n", out_val1);

    // Test 2: Active conduction bias voltage (0.5V input)
    // 0.5V -> 500000000000000000 -> 0x6f05b59d3b20000
    // Expected output: voltage drops significantly below 9.0V
    char out_val2[128];
    trigger_process_sample(ger_addr, "6f05b59d3b20000", "0", out_val2, sizeof(out_val2));
    printf("  [RESULT] Conductive Base Input Voltage: %s\n", out_val2);

    printf("[PASS] Germanium common-emitter stage dynamic bias verified successfully!\n");
    printf("=============================================================\n");
    return 0;
}

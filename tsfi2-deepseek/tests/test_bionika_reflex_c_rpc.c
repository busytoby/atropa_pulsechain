...
int main() {
    printf("=============================================================\n");
    printf("ELEKTOR ISSUE #14: BIONIKA STAPEDIUS REFLEX (ANVIL LIVE)\n");
    printf("=============================================================\n");

    printf("[C-Test] Compiling BionikaReflex Yul object...\n");
    char *ref_hex = malloc(131072);
    assert(ref_hex != NULL);
    compile_yul_to_bytecode("../solidity/bin/bionikaReflex.yul", ref_hex, 131072);

    char ref_addr[128];
    printf("[C-Test] Deploying BionikaReflex contract dynamically from C...\n");
    deploy_contract_from_c(ref_hex, ref_addr, sizeof(ref_addr));
    printf("  [DEPLOYED] BionikaReflex Address: %s\n", ref_addr);
    free(ref_hex);

    // Call processSamples in a single batch of 7 inputs:
    // 0: Input 1.0V (0xde0b6b3a7640000), isClipping: 1 -> charges Vm to 300mV. Output 1.0V (unclamped)
    // 1: Input 1.0V (0xde0b6b3a7640000), isClipping: 1 -> charges Vm to 600mV. Output 1.0V (unclamped)
    // 2: Input 1.0V (0xde0b6b3a7640000), isClipping: 1 -> charges Vm to 900mV (>800mV, fire!). Output 0.7V (clamped!)
    // 3: Input 1.0V (0xde0b6b3a7640000), isClipping: 0 -> refractory step 2. Output 73% (0.73V)
    // 4: Input 1.0V (0xde0b6b3a7640000), isClipping: 0 -> refractory step 1. Output 75.7% (0.757V)
    // 5: Input 1.0V (0xde0b6b3a7640000), isClipping: 0 -> refractory step 0. Output 78.1% (0.781V)
    // 6: Input 1.0V (0xde0b6b3a7640000), isClipping: 0 -> fully cooled down. Output back towards 1.0V
    const char *inputs =
    ...
    char out_val[2048];
    trigger_process_samples(ref_addr, "7", inputs, out_val, sizeof(out_val));

    printf("  [RESULT] Bionika Acoustic Reflex Samples (0-6):\n");
    printf("    Sample 0 (1.0V, Integrating):      %.64s\n", out_val + 2);
    printf("    Sample 1 (1.0V, Integrating):      %.64s\n", out_val + 2 + 64);
    printf("    Sample 2 (1.0V, FIRING!):          %.64s\n", out_val + 2 + 128);
    printf("    Sample 3 (1.0V, Refractory step 2): %.64s\n", out_val + 2 + 192);
    printf("    Sample 4 (1.0V, Refractory step 1): %.64s\n", out_val + 2 + 256);
    printf("    Sample 5 (1.0V, Refractory step 0): %.64s\n", out_val + 2 + 320);
    printf("    Sample 6 (1.0V, Leak Integration):  %.64s\n", out_val + 2 + 384);
    ...
    // Verify Sample 2 is attenuated (0.7V)
    assert(strcmp(sample2_hex, "00000000000000000000000000000000000000000000000009b6e64a8ec60000") == 0);

    printf("[PASS] Bionika stapedius neural acoustic reflex verified successfully!\n");
    printf("=============================================================\n");
    return 0;
}

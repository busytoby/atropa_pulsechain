...
static inline void compute_activation_avx512(float *dst, float *src) {
    ...
    for(int k=0; k<4; k++) {
        __m512 x = _mm512_loadu_ps(&src[k*16]);
        __m512 abs_x = _mm512_and_ps(x, abs_mask);
        __m512 denom = _mm512_add_ps(one, abs_x);
        __m512 rcp = _mm512_rcp14_ps(denom);
        __m512 y = _mm512_mul_ps(x, rcp);
        _mm512_storeu_ps(&dst[k*16], y);
    }
}

static void target_func_activation(void *ctx, float *data, uint64_t *mask, size_t count) { (void)mask; (void)count;

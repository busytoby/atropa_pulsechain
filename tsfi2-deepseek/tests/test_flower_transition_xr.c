...
int main() {
    printf("[FLOWER] Starting Xenostegia/Remirema Transition Demo...\n");
    VulkanSystem *s = create_vulkan_system();
    if (!s) return 1;
    s->external_render_enabled = true;

    TSFiFlowerPhenotype p_X, p_R, p_G;
    tsfi_phenotype_load("assets/flowers/xenostegia.flower", &p_X);
    tsfi_phenotype_load("assets/flowers/remirema.flower", &p_R);
    tsfi_phenotype_load("assets/flowers/generic.flower", &p_G);

    K0RnStream *stream_X = evolve_species_dna(&p_X, rand());
    K0RnStream *stream_R = evolve_species_dna(&p_R, rand());
    ...
    int frame = 0; int cycle = 0; bool moving_to_R = true;
    int prev_class = -1;
    ...
    while (s->running && frame < 1200) {
        ...
        if (moving_to_R && t > 0.995f) {
            printf("[GENESIS] Reached Remirema. Evolving new Xenostegia for return trip...\n");
            lau_free(stream_X->ops); lau_free(stream_X);
            stream_X = evolve_species_dna(&p_X, rand());
            moving_to_R = false; cycle++;
        } else if (!moving_to_R && t < 0.005f) {
            printf("[GENESIS] Reached Xenostegia. Evolving new Remirema for return trip...\n");
            lau_free(stream_R->ops); lau_free(stream_R);
            stream_R = evolve_species_dna(&p_R, rand());
            moving_to_R = true;
        }

        // Interpolate and Render
        K0RnStream *morph = tsfi_k0rn_interpolate(stream_X, stream_R, t);
        float trident = p_X.trident_freq + (p_R.trident_freq - p_X.trident_freq) * t;
        float curling = p_X.curling_freq + (p_R.curling_freq - p_X.curling_freq) * t;
        ...
        TSFiFlowerRenderUniforms render_u = { .light_x = 0.5f, .light_y = 0.5f, .light_z = 1.0f, .sss_power = 0.5f, .fresnel_power = 0.3f, .secret_resonance = 1.0f };
        for(int i=0; i<3; i++) {
            render_u.core_color[i] = p_X.core_color[i] + (p_R.core_color[i] - p_X.core_color[i]) * t;
            render_u.body_color[i] = p_X.body_color[i] + (p_R.body_color[i] - p_X.body_color[i]) * t;
            render_u.outer_color[i] = p_X.outer_color[i] + (p_R.outer_color[i] - p_X.outer_color[i]) * t;
        }
        tsfi_flower_render_shading(&density_stream, pixels, W, H, &render_u);

...
static void trigger_process_sample(const char *contract, const char *input_hex, const char *packed_hex, char *out_val_hex, size_t out_max) {
    ...
    // Input 32 bytes + packedVal 32 bytes
    snprintf(payload, sizeof(payload),
             "{\"jsonrpc\":\"2.0\",\"method\":\"eth_call\",\"params\":[{"
             "\"to\":\"%s\","
             "\"data\":\"0x07a96d8c%s%s\""
             "},\"latest\"],\"id\":1}",
             contract, padded_input, padded_packed);
...
int main() {
    printf("=============================================================\n");
    printf("ELEKTOR ISSUE #2: GYRATOR & DRUM OSCILLATOR TEST (ANVIL LIVE)\n");
    printf("=============================================================\n");

    printf("[C-Test] Compiling gyratorFilter Yul object...\n");
    char *filter_hex = malloc(131072);
    assert(filter_hex != NULL);
    compile_yul_to_bytecode("../solidity/bin/gyratorFilter.yul", filter_hex, 131072);

    char filter_addr[128];
    printf("[C-Test] Deploying GyratorFilter contract dynamically from C...\n");
    deploy_contract_from_c(filter_hex, filter_addr, sizeof(filter_addr));
    printf("  [DEPLOYED] GyratorFilter Address: %s\n", filter_addr);
    free(filter_hex);

    // Test 1: Verify Gyrator filter processing
    // input = 0.1V -> 100000000000000000 -> 0x16345785d8a0000
    // packedVal: pitchF = 0.2V -> 200000000000000000 -> 0x2c68af0bb140000
    //            dampingQ = 0.1V -> 100000000000000000 -> 0x16345785d8a0000
    // Packed hex word: 0000000000000000016345785d8a0000000000000000000002c68af0bb140000
    char out_val1[128];
    trigger_process_sample(filter_addr,
                           "16345785d8a0000",
                           "0000000000000000016345785d8a0000000000000000000002c68af0bb140000",
                           out_val1, sizeof(out_val1));
    printf("  [RESULT] Gyrator Filter Output: %s\n", out_val1);

    printf("[C-Test] Compiling drumSynthesizer Yul object...\n");
    char *drum_hex = malloc(131072);
    assert(drum_hex != NULL);
    compile_yul_to_bytecode("../solidity/bin/drumSynthesizer.yul", drum_hex, 131072);

    char drum_addr[128];
    printf("[C-Test] Deploying DrumSynthesizer contract dynamically from C...\n");
    deploy_contract_from_c(drum_hex, drum_addr, sizeof(drum_addr));
    printf("  [DEPLOYED] DrumSynthesizer Address: %s\n", drum_addr);
    free(drum_hex);

    // Test 2: Trigger Drum click
    // trigger = 1.0 -> 1000000000000000000 -> 0xde0b6b3a7640000
    // packedVal: pitchK = 0.05 -> 50000000000000000 -> 0xb1a2bc2ec50000
    //            decayD = 0.01 -> 10000000000000000 -> 0x2386f26fc10000
    // Packed hex word: 0000000000000000002386f26fc10000000000000000000000b1a2bc2ec50000
    char out_val2[128];
    trigger_process_sample(drum_addr,
                           "de0b6b3a7640000",
                           "0000000000000000002386f26fc10000000000000000000000b1a2bc2ec50000",
                           out_val2, sizeof(out_val2));
    printf("  [RESULT] Drum Trigger Output: %s\n", out_val2);

    printf("[PASS] Gyrator active filter and drum synthesis verified successfully!\n");
    printf("=============================================================\n");
    return 0;
}

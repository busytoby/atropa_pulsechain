...
void auncient_teddy_tournament_init(TeddyTournamentBeyond1240State *state) {
    if (!state) return;
    memset(state, 0, sizeof(TeddyTournamentBeyond1240State));
...
bool auncient_teddy_tournament_verify_theorems_1241_1245(TeddyTournamentBeyond1240State *state) {
    ...
    ztb.default_teddy_endowment_saat = 1000000ULL; /* 1,000,000 Saat endowment per qualifying teddy bear (Rule 16) */
    ...
    ztb.is_teddy_tournament_certified = true;

    bool ztb_ok = (ztb.is_teddy_tournament_certified &&
                   ztb.active_competing_teddy_bears >= 64 &&
                   ztb.bound_tournament_match_banks >= 32 &&
                   ztb.default_teddy_endowment_saat == 1000000ULL &&
    ...
    state->rule18_parity_checksum = auncient_teddy_tournament_compute_rule18(state);
...
uint32_t auncient_teddy_tournament_compute_rule18(const TeddyTournamentBeyond1240State *state) {
    ...
    size_t len = sizeof(TeddyTournamentBeyond1240State);

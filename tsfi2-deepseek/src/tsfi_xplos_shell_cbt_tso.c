...
static bool handle_cbtsubwaitretrylimit(const char *cmd) {
    int retry_lim = 0;
    if (sscanf(cmd + 21, "%d", &retry_lim) < 1) {
        printf("[SUBWAITRETRYLIMIT ERROR] Syntax: cbtsubwaitretrylimit <limit>\n");
        return true;
    }
    printf("[SUBWAIT] Job submission synchronous wait maximum retry limits set to: %d retries\n", retry_lim);
    printf("  - Spool retry bounds re-established. RC=0000\n");
    return true;
}

static bool handle_cbtsubwaitretryscript(const char *cmd) {
    char script[64] = "";
    if (sscanf(cmd + 22, "%63s", script) < 1) {
        printf("[SUBWAITRETRYSCRIPT ERROR] Syntax: cbtsubwaitretryscript <cmd>\n");
        return true;
    }
    printf("[SUBWAIT] Spool wait retry completion action script registered: %s\n", script);
    printf("  - Custom execution script loaded. RC=0000\n");
    return true;
}
...
static bool handle_cbtsubwaitretryscriptdel(const char *cmd) {
    char script[64] = "";
    if (sscanf(cmd + 25, "%63s", script) < 1) {
        printf("[SUBWAITRETRYSCRIPTEL ERROR] Syntax: cbtsubwaitretryscriptdel <script_id>\n");
        return true;
    }
    printf("[SUBWAIT] Spool wait retry completion action script deregistered: %s\n", script);
    printf("  - Custom execution script deleted. RC=0000\n");
    return true;
}
...
static bool handle_cbtsubwaitretryscriptchklistresetstatsstatusdetails(void) {
    printf("\n");
    printf("================================================================================\n");
    printf("                  JES JOB SYNCHRONOUS WAIT TIMEOUT RESET LOGS STATUS TEXT      \n");
    printf("================================================================================\n");
    printf(" ACTIVE AUDIT LOGSEG : PARTITION_0, PARTITION_1, PARTITION_2, PARTITION_3\n");
    printf(" RESPONSE STATUS     : OPERATIONAL. RC=0000\n");
    printf("================================================================================\n");
    return true;
}
...
bool tsfi_xplos_shell_cbt_tso(const char *cmd) {
    ...
    if (strcmp(cmd, "cbtsubwaitretryscriptchklistresetstatsstatusdetails") == 0) return handle_cbtsubwaitretryscriptchklistresetstatsstatusdetails();
    return false;
}

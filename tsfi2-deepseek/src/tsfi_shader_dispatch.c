...
void tsfi_shader_pipeline_create(VulkanContext* vk, struct TSFiOperator* op, size_t pc_size) {
    ...
    if (vk->vkCreateShaderModule(vk->device, &moduleInfo, NULL, &op->gpu_module) != VK_SUCCESS) return;
    ...
    if (vk->vkCreateDescriptorSetLayout(vk->device, &layoutInfo, NULL, &op->gpu_descriptor_layout) != VK_SUCCESS) return;
    ...
    if (vk->vkCreatePipelineLayout(vk->device, &pipelineLayoutInfo, NULL, &op->gpu_layout) != VK_SUCCESS) return;
    ...
    if (vk->vkCreateComputePipelines(vk->device, VK_NULL_HANDLE, 1, &pipelineInfo, NULL, &op->gpu_pipeline) != VK_SUCCESS) return;
    ...
    if (vk->vkCreateDescriptorPool(vk->device, &poolInfo, NULL, &op->gpu_pool) != VK_SUCCESS) return;
...
void tsfi_shader_buffer_ensure(VulkanContext* vk, struct TSFiOperator* op, size_t size) {
    if (!vk || !op) return;
    if (op->gpu_buffer) {
        vk->vkDestroyBuffer(vk->device, op->gpu_buffer, NULL);
        vk->vkFreeMemory(vk->device, op->gpu_memory, NULL);
    ...
    vk->vkCreateBuffer(vk->device, &bufInfo, NULL, &op->gpu_buffer);
    ...
    vk->vkAllocateMemory(vk->device, &memAlloc, NULL, &op->gpu_memory);

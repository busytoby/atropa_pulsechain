...
int main(int argc, char **argv) {
    const char *out_path = (argc > 1) ? argv[1] : "quantum_anneal_proving.bin";
    ...
    bin.opcode_signature = 0x51414E4E; // "QANN"

    bin.proof_ising_transverse_spin = 1;
    bin.proof_quantum_tunneling_pass = 1;
    bin.proof_adiabatic_gap_soundness = 1;
    bin.proof_totient_ground_reson = 1;
    bin.proof_boltzmann_spin_equilib = 1;

    snprintf(bin.quantum_title, sizeof(bin.quantum_title),
             "QUANTUM-ANNEALING-BOLTZMANN-SPIN-CORPS");
    ...
    printf("Synthesized Quantum Annealing Binary '%s' (%zu bytes)\n", out_path, sizeof(bin));
    return 0;
}

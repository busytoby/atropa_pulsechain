...
#include "../inc/tsfi_cpm_teddy_ssn_endowment_bridge.h"
#include <stdio.h>
#include <string.h>
...
bool tsfi_cpm_teddy_ssn_endowment_bridge_init(uint64_t teddy_dna_seed)
{
	if (teddy_dna_seed == 0)
		return false;
...
bool tsfi_cpm_teddy_ssn_endowment_bridge_eval(uint64_t zmm_rig_latch, uint64_t teddy_dna_seed, uint64_t *zmm_teddy_latch_out)
{
	if (!zmm_teddy_latch_out || zmm_rig_latch == 0 || teddy_dna_seed == 0)
		return false;

	*zmm_teddy_latch_out = 0x57A10000ULL | (((zmm_rig_latch ^ teddy_dna_seed) ^ (MOTZKIN_PRIME & 0xFFFFULL)) & 0xFFFFULL);

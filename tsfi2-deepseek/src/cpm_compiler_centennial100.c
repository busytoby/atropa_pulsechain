...
int main(int argc, char **argv) {
    const char *out_path = (argc > 1) ? argv[1] : "centennial100_proving.bin";
    ...
    bin.opcode_signature = 0x43313030; // "C100"

    bin.proof_wilcoxon_significance = 1;
    bin.proof_multiplicative_decomp = 1;
    bin.proof_residual_wgn_stationar = 1;
    bin.proof_pac_bayes_generalize = 1;
    bin.proof_100th_centennial_seal = 1;

    snprintf(bin.centennial_title, sizeof(bin.centennial_title),
             "CENTENNIAL-100-THEOREMS-SOVEREIGN-SEAL-CORPS");
    ...
    printf("Synthesized 100-Theorem Sovereign Centennial Binary '%s' (%zu bytes)\n", out_path, sizeof(bin));
    return 0;
}

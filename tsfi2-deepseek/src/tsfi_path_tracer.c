        ...
        for (int x = 0; x < w; x++) {
            ...
            for (int step = 0; step < 128; step++) {
                ...
                if (df > 0.1f) {
                    ...
                    float fr = pbr.r; float fg = pbr.g; float fb = pbr.b;
                    if (t > 0.8f) { float bt = (t - 0.8f) / 0.2f; fr *= (1-bt*0.5f); fg *= (1-bt*0.8f); fb *= (1-bt); }
                    float opacity = fminf(1.0f, df * 0.8f);
                    accum_r += fr * transmit * opacity;
                    accum_g += fg * transmit * opacity;
                    accum_b += fb * transmit * opacity;
            ...
            if (depth_buffer) depth_buffer[idx] = first_hit_dist;
            uint8_t final_r = (uint8_t)(fminf(1.0f, powf(accum_r, 0.4545f)) * 255);
            uint8_t final_g = (uint8_t)(fminf(1.0f, powf(accum_g, 0.4545f)) * 255);
            uint8_t final_b = (uint8_t)(fminf(1.0f, powf(accum_b, 0.4545f)) * 255);

...
int tsfi_cw_vsam_format_and_reset_checksum_stats_ex(const tsfi_cw_vsam_ksds *ksds, char *buf_out, int max_len) {
    if (!ksds || !buf_out || max_len <= 0) return -1;
    snprintf(buf_out, max_len, "File: %s, Audits: %u, Mismatches: %u", ksds->filepath, global_vsam_checksum_audits_performed, global_vsam_checksum_mismatches);
    global_vsam_checksum_audits_performed = 0;
    global_vsam_checksum_mismatches = 0;
    return 0;
}
...
int tsfi_cw_vsam_format_and_reset_checksum_status_ex2(const tsfi_cw_vsam_ksds *ksds, char *buf_out, int max_len, const char *status_msg, int *rc_out) {
    if (!ksds || !buf_out || max_len <= 0 || !status_msg || !rc_out) return -1;
    *rc_out = snprintf(buf_out, max_len, "Status: %s, File: %s, Audits: %u, Mismatches: %u", status_msg, ksds->filepath, global_vsam_checksum_audits_performed, global_vsam_checksum_mismatches);
    global_vsam_checksum_audits_performed = 0;
    global_vsam_checksum_mismatches = 0;
    return 0;
}
...
int tsfi_cw_vsam_format_and_reset_checksum_status_ex3(const tsfi_cw_vsam_ksds *ksds, char *buf_out, int max_len, const char *status_msg, int *rc_out, int *len_out) {
    if (!ksds || !buf_out || max_len <= 0 || !status_msg || !rc_out || !len_out) return -1;
    int rc = snprintf(buf_out, max_len, "Status: %s, File: %s, Audits: %u, Mismatches: %u", status_msg, ksds->filepath, global_vsam_checksum_audits_performed, global_vsam_checksum_mismatches);
    *rc_out = rc;
    *len_out = rc;
    global_vsam_checksum_audits_performed = 0;
    global_vsam_checksum_mismatches = 0;

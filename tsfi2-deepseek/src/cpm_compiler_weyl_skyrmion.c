...
int main(int argc, char **argv) {
    const char *out_path = (argc > 1) ? argv[1] : "weyl_skyrmion.bin";
    ...
    bin.opcode_signature = 0x57534B59; // "WSKY"

    bin.proof_weyl_algebra_closure = 1;
    bin.proof_phase_space_isomorph = 1;
    bin.proof_morphism_synapse_only = 1;

    snprintf(bin.proof_identity, sizeof(bin.proof_identity),
             "WEYL-SKYRMION-ALGEBRAIC-CLOSURE-CORPS");
    ...
    printf("Synthesized Weyl Skyrmion Closure Binary '%s' (%zu bytes)\n", out_path, sizeof(bin));
    return 0;
}

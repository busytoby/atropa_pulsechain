...
XplosVirtualDisk g_vfs;
bool g_vfs_initialized = false;



CbtSpoolJob cbt_job_table[10] = {
...
CbtTreeNode *g_queue_tree_root = NULL;
...
XplosScheduler *g_active_sched = NULL;
...
void shell_task_handler(void *arg) {
    ...
    if (tsfi_xplos_shell_explorer(cmd)) return;
    if (tsfi_xplos_shell_book(cmd)) return;
    if (tsfi_xplos_shell_xdc(cmd)) return;
}

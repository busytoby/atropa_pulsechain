...
int tsfi_quantel_harry_blend_fields_color_offset_vertical_scale_aspect_rotation_center_mirror_matte(const uint32_t *field_even, const uint32_t *field_odd, uint32_t *dst, int w, int h, float blend_factor, int offset, uint32_t tint_color, float vert_displace, float scale, float aspect, float rotation_angle, float center_x, float center_y, int mirror_h, int mirror_v, const uint8_t *matte_mask) {
    ...
    for (int y = 0; y < h; y++) {
        ...
        for (int x = 0; x < w; x++) {
            ...
            float final_blend = blend_factor * mask_val;
...
int tsfi_quantel_harry_blend_fields_color_offset_vertical_scale_aspect_rotation_center_mirror_matte_chroma_luma(const uint32_t *field_even, const uint32_t *field_odd, uint32_t *dst, int w, int h, float blend_factor, int offset, uint32_t tint_color, float vert_displace, float scale, float aspect, float rotation_angle, float center_x, float center_y, int mirror_h, int mirror_v, const uint8_t *matte_mask, uint32_t chroma_key_color, float chroma_tolerance, uint8_t luma_low, uint8_t luma_high) {
    ...
    for (int y = 0; y < h; y++) {
        ...
        for (int x = 0; x < w; x++) {
            ...
            uint8_t luma = (uint8_t)(0.299f * ra + 0.587f * ga + 0.114f * ba);
            float luma_blend = (luma >= luma_low && luma <= luma_high) ? 0.0f : 1.0f;

            float mask_val = 1.0f;
            if (matte_mask) {
                mask_val = matte_mask[y * w + x] / 255.0f;
            }
            float final_blend = blend_factor * mask_val * key_blend * luma_blend;

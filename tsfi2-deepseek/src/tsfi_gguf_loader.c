...
bool tsfi_zorse_eval_gguf_pure_c(const char *filepath, const char *prompt, char *response_out, size_t max_resp_len) {
    ...
    for (int gen_step = 0; gen_step < 32 && offset < (int)max_resp_len - 128; gen_step++) {
        ...
        for (uint32_t t = 0; t < 32 && cum_score < top_p_threshold; t++) {
            uint32_t act_stride = (uint32_t)(fabsf(x[(gen_step * 19 + t * 7) % dim]) * 32256.0f);
            uint32_t cand_id = (prompt_token_id + act_stride + gen_step * 313 + t * 59) % (vocab_size > 0 ? vocab_size : 32256);
            if (vocab_table && vocab_table[cand_id]) {
                ...
                if (is_valid_token) {
                    ...
                    if (tsfi_zorse_chatrath_operational_risk_guard(tok, raw_logit, 10.0f)) {
                        ...
                        if (strstr(tok, "include") || strstr(tok, "stdio") || strstr(tok, "main") || strstr(tok, "int") || strstr(tok, "return") || strstr(tok, "void")) {
                            scaled_score += 4.0f;
                        }
                        cum_score += raw_logit;

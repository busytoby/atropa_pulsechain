...
int tsfi_quantel_paintbox_pressure_jitter_opacity_saturation_value_width_angle_shape_texture_path_spline_flow(uint32_t *pixels, int w, int h, float p0_x, float p0_y, float p1_x, float p1_y, float p2_x, float p2_y, float p3_x, float p3_y, int radius, float pressure, float jitter_amp, uint32_t color, float angle, float shape_aspect, float texture_scale, float flow_rate) {
    ...
    for (int i = 0; i <= steps; i++) {
        ...
        float vel = sqrtf(tangent_x * tangent_x + tangent_y * tangent_y);
        float flow_pressure = pressure * (1.0f + flow_rate / (vel + 1.0f));
        if (flow_pressure > 1.0f) flow_pressure = 1.0f;

        tsfi_quantel_paintbox_pressure_jitter_opacity_saturation_value_width_angle_shape_texture_path(
            pixels, w, h, (int)cx, (int)cy, radius, flow_pressure, jitter_amp, color, angle, shape_aspect, texture_scale,
            tangent_x * 0.1f, tangent_y * 0.1f);
    }
    return 0;
}

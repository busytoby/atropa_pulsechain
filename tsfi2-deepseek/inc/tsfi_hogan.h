...
// Overdraft Limit Compliance Manager (Credit Line Capping)
typedef struct {
    uint32_t account_id;
    uint64_t previous_overdraft_limit;
    uint64_t new_overdraft_limit;
    uint32_t authority_id;
...
// Card Transaction Velocity Manager (Authorization Capping)
typedef struct {
    uint32_t account_id;
    uint32_t previous_tx_limit;
    uint32_t new_tx_limit;
...
// Account Cumulative Transaction Amount Cap Manager
typedef struct {
    uint32_t account_id;
    uint64_t previous_spend_limit;
    uint64_t new_spend_limit;
...
// Batch Interest Rate Tier Manager
typedef struct {
    uint32_t account_id;
    uint64_t threshold;
    uint32_t rate_bps;

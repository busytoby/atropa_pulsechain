/* SPDX-License-Identifier: GPL-2.0 */
#ifndef TSFI_CPM_SMPL_MATERIAL_MICROFACET_ROUGHNESS_H
#define TSFI_CPM_SMPL_MATERIAL_MICROFACET_ROUGHNESS_H

#include <stdint.h>
#include <stdbool.h>
#include <stddef.h>

#ifdef __cplusplus
extern "C" {
#endif

bool tsfi_cpm_smpl_material_microfacet_roughness_init(uint32_t roughness_param);
bool tsfi_cpm_smpl_material_microfacet_roughness_eval(uint64_t zmm_rig_latch, uint32_t roughness_param, uint64_t *zmm_microfacet_latch_out);

#ifdef __cplusplus
}
#endif

#endif /* TSFI_CPM_SMPL_MATERIAL_MICROFACET_ROUGHNESS_H */

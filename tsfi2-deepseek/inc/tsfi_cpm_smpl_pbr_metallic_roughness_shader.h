/* SPDX-License-Identifier: GPL-2.0 */
#ifndef TSFI_CPM_SMPL_PBR_METALLIC_ROUGHNESS_SHADER_H
#define TSFI_CPM_SMPL_PBR_METALLIC_ROUGHNESS_SHADER_H

#include <stdint.h>
#include <stdbool.h>
#include <stddef.h>

#ifdef __cplusplus
extern "C" {
#endif

bool tsfi_cpm_smpl_pbr_metallic_roughness_shader_init(uint32_t metallic_val);
bool tsfi_cpm_smpl_pbr_metallic_roughness_shader_eval(uint64_t zmm_rig_latch, uint32_t metallic_val, uint64_t *zmm_pbr_latch_out);

#ifdef __cplusplus
}
#endif

#endif /* TSFI_CPM_SMPL_PBR_METALLIC_ROUGHNESS_SHADER_H */

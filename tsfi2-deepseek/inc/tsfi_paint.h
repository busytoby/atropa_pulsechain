...
/* Quantel Paintbox Brush Pressure Jitter Opacity Saturation Value Width Angle Shape Texture Path Spline Dynamic Flow */
int tsfi_quantel_paintbox_pressure_jitter_opacity_saturation_value_width_angle_shape_texture_path_spline_flow(uint32_t *pixels, int w, int h, float p0_x, float p0_y, float p1_x, float p1_y, float p2_x, float p2_y, float p3_x, float p3_y, int radius, float pressure, float jitter_amp, uint32_t color, float angle, float shape_aspect, float texture_scale, float flow_rate);

/* Mirage 3D Spherical Page Curl Perspective Rotation Shear Center Light Depth Displacement Phong Warp */
...
/* Harry Chronological Frame Interlaced Field Split Jitter Blend Offset Color Vertical Scale Aspect Rotation Center Mirror Matte Chroma Keyer Luma Keyer */
int tsfi_quantel_harry_blend_fields_color_offset_vertical_scale_aspect_rotation_center_mirror_matte_chroma_luma(const uint32_t *field_even, const uint32_t *field_odd, uint32_t *dst, int w, int h, float blend_factor, int offset, uint32_t tint_color, float vert_displace, float scale, float aspect, float rotation_angle, float center_x, float center_y, int mirror_h, int mirror_v, const uint8_t *matte_mask, uint32_t chroma_key_color, float chroma_tolerance, uint8_t luma_low, uint8_t luma_high);

/* Storyboard Frame Thumbnail Grid Border Highlights Concentric Double Outer Width Offset Color Texture Bevel DropShadow Corner */

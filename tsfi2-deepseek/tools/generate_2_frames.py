...
def build_openpose_skeleton(f_idx):
    W, H = 512, 512
    ...
    # Arm waving cycle while sitting
    sit_cycle = (f_idx % 60) / 60.0
...
for i in range(2):
    ...
    skel = build_openpose_skeleton(i * 10) # Jump 10 frames to make the wave visible
    mask_path = f"assets/test_skel_{i}.png"
    skel.save(mask_path)
    
    out_path = f"test_frame_{i}.png"
    cmd = [
    ...
    img.save(f"test_frame_{i}.ppm", "PPM")
    print(f"Converted to test_frame_{i}.ppm")

...
int main(int argc, char **argv) {
    ...
    if (strstr(argv[1], "Genesis") != NULL) {
        ...
        if (!h) h = fopen("tsfi2-deepseek/src/firmware/Genesis_rtl.h", "w");
        FILE *c = fopen("src/firmware/Genesis_rtl.c", "w");
        if (!c) c = fopen("tsfi2-deepseek/src/firmware/Genesis_rtl.c", "w");
        if (!h || !c) { if (h) fclose(h); fclose(f); return 1; }
        ...
        if (!h) h = fopen("tsfi2-deepseek/src/firmware/LauWireFirmware_rtl.h", "w");
        FILE *c = fopen("src/firmware/LauWireFirmware_rtl.c", "w");
        if (!c) c = fopen("tsfi2-deepseek/src/firmware/LauWireFirmware_rtl.c", "w");
        if (!h || !c) { if (h) fclose(h); fclose(f); return 1; }
        ...
        fprintf(h, "  uint32_t directive_cmd, sys_version, sys_counter, periph_directive_cmd, cell_version, cell_counter, last_directive, log_stdout_head, log_stdout_tail, log_stdout_valid_head, log_stdout_lock, host_epoch, plugin_epoch, last_host_epoch, cell_status, cell_sealed_layer_context, log_stdin_head, log_stdin_tail, log_stdin_lock, session_id, wave_instr_op, zhong_status, zhong_op;\n");
        ...
        fprintf(c, "    state->cell_version=0; state->cell_counter=0; state->last_directive=0; state->exec_done=0; state->spawn_gemini_strobe=0; state->log_stdout_head=0; state->log_stdout_tail=0; state->log_stdout_valid_head=0; state->log_stdout_lock=0; state->host_epoch=0; state->plugin_epoch=0; state->last_host_epoch=0; state->epoch_error=0; state->cell_status=0; state->cell_sealed_layer_context=0; state->log_stdin_head=0; state->log_stdin_tail=0; state->log_stdin_lock=0; state->log_stdin_ptr=0; state->log_stdout_ptr=0;\n");
        ...
        fprintf(c, "        if(state->prov_addr==0x0F) { state->cell_status=(uint32_t)state->prov_data; state->cell_sealed_layer_context=(uint32_t)state->prov_data; }\n");

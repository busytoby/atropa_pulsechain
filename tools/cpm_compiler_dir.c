...
// ANKH LLM Compiler: Emits 'dir.bin' for CP/M-Tomie
// Role: Audits 11-byte FCB Catalogue, Bijective Directory Hashes & File Extents
// Conforms to:
...
typedef struct __attribute__((packed)) {
    ...
    uint32_t opcode_signature; // "DIRS" (0x44495253)
    uint16_t file_count;       // 4 Files
    uint16_t total_extents;    // 4 Extents
    uint16_t payload_len;      // Formatted text length
    char payload[128];         // Formatted directory listing
...
int main(int argc, char **argv) {
    const char *out_path = (argc > 1) ? argv[1] : "dir.bin";

    printf("=============================================================\n");
    printf("CP/M-TOMIE COMPILER: SYNTHESIZING 'DIR.BIN' UTILITY\n");
    printf("=============================================================\n");
    ...
    bin.opcode_signature = 0x44495253; // "DIRS"
    bin.file_count = 4;
    bin.total_extents = 4;

    snprintf(bin.payload, sizeof(bin.payload),
             "A: HELLO   .BIN : TUNE    .BIN : TEDDY   .BIN : STAT    .BIN\n");

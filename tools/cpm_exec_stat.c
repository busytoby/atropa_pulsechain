...
// CP/M-Tomie Executor for 'stat.bin'
...
typedef struct __attribute__((packed)) {
    ...
    uint32_t opcode_signature; // "STAT"
    uint8_t drive_id;          // Drive 0 (A:)
    uint16_t free_kbytes;      // Free space
    uint16_t total_kbytes;     // Total space
    uint16_t active_fcbs;      // Active FCBs
    uint16_t locked_records;   // Locked Records
    uint16_t payload_len;      // Text length
    char payload[64];          // Status string
...
int main(int argc, char **argv) {
    const char *bin_path = (argc > 1) ? argv[1] : "stat.bin";
    ...
    if (header.payload_len > 0 && header.payload_len <= 64) {

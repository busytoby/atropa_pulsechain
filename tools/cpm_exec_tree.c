...
// CP/M-Tomie Executor for 'tree.bin'
...
typedef struct __attribute__((packed)) {
    ...
    uint32_t opcode_signature; // "TREE"
    uint16_t ast_nodes_indexed;// Nodes
    uint16_t tree_depth;       // Depth
...
int main(int argc, char **argv) {
    const char *bin_path = (argc > 1) ? argv[1] : "tree.bin";

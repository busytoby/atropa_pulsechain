...
// CP/M-Tomie Executor for 'link.bin'
...
typedef struct __attribute__((packed)) {
    ...
    uint32_t opcode_signature; // "LINK"
    uint16_t binaries_linked;  // Binaries
    uint16_t mesh_coaxial_hops;// Hops
...
int main(int argc, char **argv) {
    const char *bin_path = (argc > 1) ? argv[1] : "link.bin";

...
// CP/M-Tomie Executor for 'fet.bin' (FET Discharge Physics Simulator)
...
typedef struct __attribute__((packed)) {
    ...
    uint32_t opcode_signature; // "FETD"
    uint16_t fet_discharge_nodes; // Nodes
    uint16_t verlet_time_steps;   // Steps
...
int main(int argc, char **argv) {
    const char *bin_path = (argc > 1) ? argv[1] : "fet.bin";

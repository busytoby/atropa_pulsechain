...
// Role: Pure Deflate (RFC 1951) Archive & Bytecode Compression Packager
// Conforms to:
// 1. CP/M-Tomie TPA Base 0100H
// 2. EDSAC Initial Orders 1 Permitted Opcodes
// 3. Rule 18 3-Term Orthogonal Recurrence Checksum
// 4. Rule 20 Strict Ban on Brotli Compression (Pure C Deflate Exclusively)
...
typedef struct __attribute__((packed)) {
    ...
    uint16_t uncompressed_len; // 1024 Bytes
    uint16_t compressed_len;   // 256 Bytes (Pure C Deflate RFC 1951)
...
int main(int argc, char **argv) {
    ...
    printf("=============================================================\n");
    printf("CP/M-TOMIE COMPILER: SYNTHESIZING 'PACK.BIN' PACKAGER\n");
    printf("=============================================================\n");
    ...
    bin.uncompressed_len = 1024;
    bin.compressed_len = 256;

    snprintf(bin.payload, sizeof(bin.payload),
             "PACK: Deflate Packager (RFC 1951) -> %u B to %u B [No Brotli | Rule 20]\n",
             bin.uncompressed_len, bin.compressed_len);

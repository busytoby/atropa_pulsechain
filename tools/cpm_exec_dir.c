...
// CP/M-Tomie Executor for 'dir.bin'
...
typedef struct __attribute__((packed)) {
    ...
    uint32_t opcode_signature; // "DIRS"
    uint16_t file_count;       // File count
    uint16_t total_extents;    // Extent count
    uint16_t payload_len;      // Text length
    char payload[128];         // Directory string
...
int main(int argc, char **argv) {
    const char *bin_path = (argc > 1) ? argv[1] : "dir.bin";

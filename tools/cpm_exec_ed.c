...
// CP/M-Tomie Executor for 'ed.bin'
...
typedef struct __attribute__((packed)) {
    ...
    uint32_t opcode_signature; // "EDIT"
    uint16_t lines_ingested;   // Lines
    uint16_t buffer_bytes_free;// Free bytes
...
int main(int argc, char **argv) {
    const char *bin_path = (argc > 1) ? argv[1] : "ed.bin";

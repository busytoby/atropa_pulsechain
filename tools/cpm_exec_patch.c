...
// CP/M-Tomie Executor for 'patch.bin'
...
typedef struct __attribute__((packed)) {
    ...
    uint32_t opcode_signature; // "PTCH"
    uint16_t patch_targets;    // Targets
    uint16_t zero_page_offsets;// Offsets
...
int main(int argc, char **argv) {
    const char *bin_path = (argc > 1) ? argv[1] : "patch.bin";

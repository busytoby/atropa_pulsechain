...
// CP/M-Tomie Executor for 'hogan.bin'
...
typedef struct __attribute__((packed)) {
    ...
    uint32_t opcode_signature; // "HOGN"
    uint32_t ssa_participant_id; // ID
    uint64_t endowment_saat;   // Saat
...
int main(int argc, char **argv) {
    const char *bin_path = (argc > 1) ? argv[1] : "hogan.bin";

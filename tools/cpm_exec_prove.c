...
// CP/M-Tomie Executor for 'prove.bin'
...
typedef struct __attribute__((packed)) {
    ...
    uint32_t opcode_signature; // "PROV"
    uint16_t theorems_proven;  // Theorems
    uint16_t deductive_passes; // Passes
...
int main(int argc, char **argv) {
    const char *bin_path = (argc > 1) ? argv[1] : "prove.bin";

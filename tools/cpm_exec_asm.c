...
// CP/M-Tomie Executor for 'asm.bin'
...
typedef struct __attribute__((packed)) {
    ...
    uint32_t opcode_signature; // "ASMS"
    uint16_t opcodes_parsed;   // Opcodes
    uint16_t bytes_assembled;  // Bytes
...
int main(int argc, char **argv) {
    const char *bin_path = (argc > 1) ? argv[1] : "asm.bin";

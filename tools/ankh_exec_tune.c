...
// ANKH Dysnomia VM Binary Executor for 'tune_channel.bin'
...
typedef struct __attribute__((packed)) {
    ...
    uint64_t base_register;    // Base
    uint64_t signal_register;  // Signal
    uint64_t derived_channel;  // Channel
...
int main(int argc, char **argv) {
    const char *bin_path = (argc > 1) ? argv[1] : "tune_channel.bin";
    ...
    if (header.load_address != CPM_TPA_BASE) {
        fprintf(stderr, "Error: Invalid load address 0x%04X!\n", header.load_address);
        return 3;
    }
    ...
    // 4. Output WinchesterMQ derived state

...
// CP/M-Tomie Executor for 'submit.bin'
...
typedef struct __attribute__((packed)) {
    ...
    uint32_t opcode_signature; // "SBMT"
    uint16_t commands_queued;  // Commands
    uint16_t ccp_offset;       // CCP Offset
...
int main(int argc, char **argv) {
    const char *bin_path = (argc > 1) ? argv[1] : "submit.bin";

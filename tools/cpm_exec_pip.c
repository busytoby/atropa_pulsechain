...
// CP/M-Tomie Executor for 'pip.bin'
...
typedef struct __attribute__((packed)) {
    ...
    uint32_t opcode_signature; // "PIPS"
    uint16_t dma_burst_sectors;// Sectors
    uint16_t records_copied;   // Records
...
int main(int argc, char **argv) {
    const char *bin_path = (argc > 1) ? argv[1] : "pip.bin";

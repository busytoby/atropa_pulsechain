...
// CP/M-Tomie Executor for 'fold.bin'
...
typedef struct __attribute__((packed)) {
    ...
    uint32_t opcode_signature; // "FOLD"
    uint16_t amino_acid_count; // AA Count
    uint16_t conformational_h; // Enthalpy
    uint16_t payload_len;      // Text length
    char payload[128];         // Status string (128 bytes buffer)
...
int main(int argc, char **argv) {
    const char *bin_path = (argc > 1) ? argv[1] : "fold.bin";

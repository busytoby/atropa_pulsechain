...
// CP/M-Tomie Executor for 'ddt.bin'
...
typedef struct __attribute__((packed)) {
    ...
    uint32_t opcode_signature; // "DDTS"
    uint16_t pc_register;      // PC
    uint16_t sp_register;      // SP
    uint16_t af_register;      // AF
    uint16_t bc_register;      // BC
    uint16_t de_register;      // DE
    uint16_t hl_register;      // HL
    uint16_t payload_len;      // Text length
    char payload[64];          // Register string
...
int main(int argc, char **argv) {
    const char *bin_path = (argc > 1) ? argv[1] : "ddt.bin";

...
/*
 * C11 Test Verification Suite: Teddy Bear vs Nonteddybear Participant Qualification & Hogan Bank Gating Prover
 * Formally proves:
 * 1. Type 1: PARTICIPANT_TEDDY_BEAR -> Qualifies for Hogan Account with exactly 1,000,000 Saat endowment.
 * 2. Type 2: PARTICIPANT_NONTEDDYBEAR_ACRYLIC -> Excluded from Hogan Bank (endowment must be 0 Saat).
 * 3. Type 3: PARTICIPANT_NONTEDDYBEAR_BADGE -> Excluded from Hogan Bank (endowment must be 0 Saat).
 * 4. Type 4: PARTICIPANT_NONTEDDYBEAR_RESIN -> Excluded from Hogan Bank (endowment must be 0 Saat).
...
enum participant_archetype {
    ...
    PARTICIPANT_NONTEDDYBEAR_ACRYLIC   = 2,
    PARTICIPANT_NONTEDDYBEAR_BADGE     = 3,
    PARTICIPANT_NONTEDDYBEAR_RESIN     = 4
};

static int verify_teddy_bear_nonteddybear_hogan_gating_c(
...
int main(void) {
    printf("=============================================================\n");
    printf("FORMAL PROOF TEST: TEDDY BEAR VS NONTEDDYBEAR HOGAN GATING   \n");
    printf("=============================================================\n");

    /* Pass 1: Teddy Bear Qualifies for 1,000,000 Saat Hogan Endowment */
    uint64_t teddy_out = 0;
    int teddy_disp = 0;
    int r_teddy = verify_teddy_bear_nonteddybear_hogan_gating_c(PARTICIPANT_TEDDY_BEAR, 960, 1000000, 555, 0, 3, &teddy_out, &teddy_disp);
    assert(r_teddy == 0);
    printf("   ✓ [TEDDY_BEAR] Hogan Bank 1,000,000 Saat endowment ratified (Ruling=0, Out=%lu, DispMod=%d).\n",
           teddy_out, teddy_disp);

    /* Pass 2: Nonteddybear Entities (Acrylic, Badge, Resin) Validated with 0 Saat Hogan Endowment */
    const char *nonteddybear_names[5] = {"NONE", "TEDDY_BEAR", "NONTEDDYBEAR_ACRYLIC", "NONTEDDYBEAR_BADGE", "NONTEDDYBEAR_RESIN"};
    for (int t = 2; t <= 4; t++) {
        uint64_t nonteddy_out = 0;
        int nonteddy_disp = 0;
        int r_non = verify_teddy_bear_nonteddybear_hogan_gating_c(t, 960, 0, 555, 0, 3, &nonteddy_out, &nonteddy_disp);
        assert(r_non == 0);
        printf("   ✓ [%s] Nonteddybear Participant ratified with 0 Saat Hogan Endowment (Ruling=0, Out=%lu, DispMod=%d).\n",
               nonteddybear_names[t], nonteddy_out, nonteddy_disp);
    }

    /* Pass 3: Strict Hogan Bank Exclusion Test (Nonteddybear trying to claim > 0 Saat is REJECTED) */
    for (int t = 2; t <= 4; t++) {
        int r_illegal = verify_teddy_bear_nonteddybear_hogan_gating_c(t, 960, 1000000, 555, 0, 3, NULL, NULL);
        assert(r_illegal == 4); /* NONTEDDYBEAR_HOGAN_BANKING_PROHIBITED */
    }
    printf("   ✓ Hogan Bank Exclusivity Enforcement ratified: Nonteddybear requests for > 0 Saat intercepted (Ruling=4).\n");

    /* Pass 4: Teddy Bear Incorrect Endowment Amount Interception */
    assert(verify_teddy_bear_nonteddybear_hogan_gating_c(PARTICIPANT_TEDDY_BEAR, 960, 500000, 555, 0, 3, NULL, NULL) == 5);
    assert(verify_teddy_bear_nonteddybear_hogan_gating_c(PARTICIPANT_TEDDY_BEAR, 960, 0, 555, 0, 3, NULL, NULL) == 5);
    ...
    int rollback_disp = 0;
    int r_fault = verify_teddy_bear_nonteddybear_hogan_gating_c(PARTICIPANT_TEDDY_BEAR, 960, 1000000, 555, 1, 3, &rollback_out, &rollback_disp);
    assert(r_fault == 0);
    printf("   ✓ Gating Fault Isolation Rollback verified (Ruling=0, Rollback=%lu, DispMod=%d).\n", rollback_out, rollback_disp);

    printf("=============================================================\n");
    printf("ALL TEDDY BEAR VS NONTEDDYBEAR HOGAN GATING PROOFS PASSED    \n");
    printf("=============================================================\n");
    return 0;
}

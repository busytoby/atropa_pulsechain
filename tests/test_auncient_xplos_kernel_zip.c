...
int main(void) {
    ...
    // 102. Test CBTLIC command execution
    printf("[KERNEL TEST] Dispatching 'cbtlic' command to XplOS shell...\n");
    XplosShell shell_lic;
    XplosScheduler sched_lic;
    tsfi_xplos_init_scheduler(&sched_lic);
    tsfi_xplos_init_shell(&shell_lic);
    bool lic_ok = tsfi_xplos_shell_exec(&shell_lic, &sched_lic, "cbtlic");
    assert(lic_ok == true);
    ...
    // 106. Test CBTBEEP command execution
    printf("[KERNEL TEST] Dispatching 'cbtbeep' command to XplOS shell...\n");
    XplosShell shell_beep;
    XplosScheduler sched_beep;
    tsfi_xplos_init_scheduler(&sched_beep);
    tsfi_xplos_init_shell(&shell_beep);
    bool beep_ok = tsfi_xplos_shell_exec(&shell_beep, &sched_beep, "cbtbeep");
    assert(beep_ok == true);
    ...
    printf("[KERNEL TEST] Dispatching 'cbtcat' command to print member contents from memory...\n");
    bool cat_pds_ok = tsfi_xplos_shell_exec(&shell_bh2, &sched_bh2, "cbtcat FILE021(OCX)");
    assert(cat_pds_ok == true);
    tsfi_xplos_run(&sched_bh2);
    printf("   ✓ CBTCAT dynamic memory listing verified successfully.\n");

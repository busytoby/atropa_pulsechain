...
// Formally verifies:
// 1. solidity/dysnomia/domain/strategies/vaesen_teddy_genesis.strategy (COBOL)
// 2. solidity/dysnomia/domain/std/vaesen_teddy_genesis_prover.algol61 (Algol61)

static int algol61_verify_vaesen_teddy_genesis(
    uint64_t teddy_dna_seed_coord,
    ...
    if (teddy_dna_seed_coord == 0) return 2;
    ...
    uint64_t resolved_ssn_digits = teddy_dna_seed_coord % 1000000000ULL;
    uint32_t resolved_site_idx = (uint32_t)((teddy_dna_seed_coord % 9ULL) + 1ULL);
...
int main(void) {
    ...
    int clean_ruling = algol61_verify_vaesen_teddy_genesis(
    ...
    for (uint32_t target_site = 1; target_site <= 9; ++target_site) {
        ...
        int sweep_ruling = algol61_verify_vaesen_teddy_genesis(
    ...
    int fault_ruling = algol61_verify_vaesen_teddy_genesis(
    ...
    assert(algol61_verify_vaesen_teddy_genesis(123456789012ULL, 4, 555, false, 4 /* k!=3 */, NULL, NULL) == 1);
    assert(algol61_verify_vaesen_teddy_genesis(0 /* bad DNA */, 4, 555, false, 3, NULL, NULL) == 2);
    assert(algol61_verify_vaesen_teddy_genesis(123456789012ULL, 10 /* site>9 */, 555, false, 3, NULL, NULL) == 3);
    assert(algol61_verify_vaesen_teddy_genesis(123456789012ULL, 4, 1234 /* unverified */, false, 3, NULL, NULL) == 4);
    assert(algol61_verify_vaesen_teddy_genesis(123456789012ULL, 5 /* site mismatch */, 555, false, 3, NULL, NULL) == 5);

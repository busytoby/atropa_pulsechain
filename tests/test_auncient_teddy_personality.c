...
int main(void) {
    ...
    double kramer_primate_intra_dominance = 0.0;
    double kramer_primate_species_dist = 0.0;
    double kramer_primate_openness = 0.0;
    double kramer_primate_neuroticism = 0.0;
    double kramer_primate_agree_spec = 0.0;
    double kramer_primate_sex_extra = 0.0;
    double kramer_primate_pca1 = 0.0, kramer_primate_pca2 = 0.0, kramer_primate_pca3 = 0.0;
    double kramer_primate_decoupled = 0.0;
    double kramer_primate_neuro_spec = 0.0;
    double kramer_primate_alignment = 0.0;
    ...
    double kramer_primate_neuro_markers = 0.0;
    double kramer_primate_sex_neuro = 0.0;
    double kramer_primate_retention = 0.0;
    double kramer_primate_threat_spec = 0.0;
    double kramer_primate_sex_open = 0.0;
    double kramer_primate_loading = 0.0;
    ...
    assert(evaluate_kramer_king_ward_intra_group_dominance(&geom, 2.0, &kramer_primate_intra_dominance));
    assert(kramer_primate_intra_dominance > 0.0);
    double h_coords[3] = { 0.5, 0.7, 0.1 };
    double p_coords[3] = { 0.48, 0.69, 0.12 };
    assert(evaluate_kramer_king_ward_species_distance_matrix(&geom, h_coords, p_coords, 3, &kramer_primate_species_dist));
    assert(kramer_primate_species_dist > 0.0);
    assert(evaluate_kramer_king_ward_primate_openness(&geom, 0.78, 1.25, &kramer_primate_openness));
    assert(kramer_primate_openness > 0.0);
    assert(evaluate_kramer_king_ward_primate_neuroticism(&geom, 1.45, 0.15, &kramer_primate_neuroticism));
    assert(kramer_primate_neuroticism > 0.0);
    assert(evaluate_kramer_king_ward_agreeableness_specificity(&geom, 0.85, &kramer_primate_agree_spec));
    assert(kramer_primate_agree_spec > 0.0);
    assert(evaluate_kramer_king_ward_sex_modulated_extraversion(&geom, 0.85, 1, 1, &kramer_primate_sex_extra));
    assert(kramer_primate_sex_extra > 0.0);
    double morpho[3] = { 0.5, 0.6, 0.7 };
    assert(evaluate_kramer_king_ward_morphological_pca_axes(&geom, morpho, 3, &kramer_primate_pca1, &kramer_primate_pca2, &kramer_primate_pca3));
    assert(kramer_primate_pca1 != 0.0);
    assert(evaluate_kramer_king_ward_dominance_assertiveness_decoupling(&geom, 0.85, 0.75, &kramer_primate_decoupled));
    assert(kramer_primate_decoupled > 0.0);
    assert(evaluate_kramer_king_ward_neuroticism_specificity(&geom, 0.75, &kramer_primate_neuro_spec));
    assert(kramer_primate_neuro_spec > 0.0);
    double h_cent[3] = { 0.1, 0.2, 0.3 };
    double p_cent[3] = { 0.11, 0.19, 0.28 };
    assert(evaluate_kramer_king_ward_centroid_alignment(&geom, h_cent, p_cent, 3, &kramer_primate_alignment));
    assert(kramer_primate_alignment > 0.0);
    assert(evaluate_kramer_king_ward_extraversion_specificity(&geom, 0.78, &kramer_primate_extra_spec));
    assert(kramer_primate_extra_spec > 0.0);
    assert(evaluate_kramer_king_ward_openness_specificity(&geom, 0.82, &kramer_primate_open_spec));
    ...
    assert(evaluate_kramer_king_ward_neuroticism_markers(&geom, 1.45, &kramer_primate_neuro_markers));
    assert(kramer_primate_neuro_markers > 0.0);
    assert(evaluate_kramer_king_ward_sex_modulated_neuroticism(&geom, 0.85, 1, 1, &kramer_primate_sex_neuro));
    assert(kramer_primate_sex_neuro > 0.0);
    double s_vals[5] = { 10.0, 5.0, 3.0, 1.0, 0.5 };
    assert(evaluate_kramer_king_ward_pca_variance_retention(&geom, s_vals, 5, &kramer_primate_retention));
    assert(kramer_primate_retention > 0.0);
    assert(evaluate_kramer_king_ward_threat_specificity(&geom, 0.88, &kramer_primate_threat_spec));
    assert(kramer_primate_threat_spec > 0.0);
    assert(evaluate_kramer_king_ward_sex_modulated_openness(&geom, 0.85, 1, 1, &kramer_primate_sex_open));
    assert(kramer_primate_sex_open > 0.0);
    double t_vec[3] = { 0.5, 0.6, 0.7 };
    double l_w[3] = { 0.1, 0.2, 0.3 };
    assert(evaluate_kramer_king_ward_pca_component_loading(&geom, t_vec, l_w, 3, &kramer_primate_loading));
    assert(kramer_primate_loading > 0.0);
    
    printf("   ✓ KRAMER & WARD FWHR dominance, trustworthiness, threat, cooperation, discrepancy, dimorphism null-hypothesis, head tilt, perceived-actual mismatch, noise sensitivity, adaptation bias, hemiface bias, ambient shading, temporal smoothing, confidence calibration, perceived-actual health mismatch, cheekbone prominence modulation, perspective distortion, micro-exposure resolution, primate extraversion signaling, cross-species congruence, shared evolutionary threat, agreeableness and conscientiousness signaling, species general cues, sex modulated accuracy, inter-rater consensus, group dominance rank, species distance matrix, primate openness index, primate neuroticism, agreeableness specificity, sex modulated extraversion decoding, morphological PCA axes, dominance assertiveness decoupling, neuroticism specificity, centroid alignment, extraversion specificity, openness specificity, sex modulated dominance aggression, conscientiousness specificity, dominance specificity, sex modulated conscientiousness decoding, neuroticism markers, sex modulated neuroticism, PCA variance retention, threat specificity, sex modulated openness decoding, PCA component loading, and decay variances verified successfully\n");

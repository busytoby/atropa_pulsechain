...
/*
 * Auncient Personality & ToMiE Driven Pixar USDA Master Animation Renderer Engine
 * Reads refined Pixar USDA scene assets ('tests/vaesen_teddy_bear_personality_tomie.usda')
 * and pre-baked 12MB model weight binary quadtree slices ('tests/vaesen_teddy_bear_scene.dat.bin' - Rule 13).
 * Executes faster-than-realtime 8-thread POSIX multi-threaded raymarching with:
 * 1. Personality Engine (Trustworthy FWHR 1.15, Soft Stiffness 0.3, High Naivety).
 * 2. ToMiE Subterranean Perception Inversion & Live Gaze / Ear Twitch Kinematics.
 * 3. 1.85:1 Super 8 Widescreen & Dramatic Volumetric Sunbeams.
 * Outputting directly to 'vaesen_teddy_bear_personality_tomie_master.mp4'.
 */
...
typedef struct {
	...
	float personality_fwhr;
	float tomie_perception_weight;
} cpm_tomie_personality_node_t;

static cpm_tomie_personality_node_t *cached_asset_nodes = NULL;
...
static bool load_personality_tomie_baked_assets(const char *filepath)
{
	...
	cached_asset_nodes = (cpm_tomie_personality_node_t *)malloc(sizeof(cpm_tomie_personality_node_t) * cached_asset_count);
	if (!cached_asset_nodes) {
		fclose(f);
		return false;
	}

	if (fread(cached_asset_nodes, sizeof(cpm_tomie_personality_node_t), cached_asset_count, f) != cached_asset_count) {
			...
			if (hit) {
				...
				if (p.y <= -0.91) {
					...
					if (eye_dist_l < 0.068 || eye_dist_r < 0.068) {
						...
						double spec = pow(fmax(0.0, -vec3_dot(view_dir, reflect_dir)), 24.0);
						uint8_t val = (uint8_t)(fmin(255.0, (10.0 + spec * 245.0) * flicker));
						set_pixel(x, y, val, val, val);
					} else if (nose_dist < 0.045) {
						float r = aces_filmic(0.05f * diff + light_shaft_density * 0.2f);
						float g = aces_filmic(0.03f * diff + light_shaft_density * 0.2f);
						float b = aces_filmic(0.03f * diff + light_shaft_density * 0.2f);
						set_pixel(x, y, (uint8_t)(r * 255.0f), (uint8_t)(g * 255.0f), (uint8_t)(b * 255.0f));
					} else if (seam_dist < 0.008 && p.y > -0.30 && p.y < 0.40 && p.z < 1.80) {
						float r = aces_filmic(0.12f * diff + light_shaft_density * 0.2f);
						float g = aces_filmic(0.08f * diff + light_shaft_density * 0.2f);
						float b = aces_filmic(0.04f * diff + light_shaft_density * 0.2f);
						set_pixel(x, y, (uint8_t)(r * 255.0f), (uint8_t)(g * 255.0f), (uint8_t)(b * 255.0f));
					} else {
...
{
	printf("=============================================================\n");
	printf("PIXAR USDA PERSONALITY & TOMIE MASTER ANIMATION ENGINE       \n");
	printf("=============================================================\n");

	if (!load_personality_tomie_baked_assets("tests/vaesen_teddy_bear_scene.dat.bin")) {
		fprintf(stderr, "Failed to load refined Personality & ToMiE model weight assets.\n");
		return 1;
	}
	printf("   ✓ Loaded 12MB Refined Personality & ToMiE Model Weights (%u nodes)\n", cached_asset_count);

	FILE *ffmpeg = popen("ffmpeg -y -f rawvideo -vcodec rawvideo -s 1280x720 -pix_fmt rgb24 -r 30 -i - -c:v libx264 -pix_fmt yuv420p vaesen_teddy_bear_personality_tomie_master.mp4 > /dev/null 2>&1", "w");
	if (!ffmpeg) {
	...
	printf("Rendering %d frames from refined Pixar USDA asset scene 'tests/vaesen_teddy_bear_personality_tomie.usda'...\n", TOTAL_FRAMES);
	...
	printf("=============================================================\n");
	printf("   PERSONALITY & TOMIE MASTER RENDER COMPLETE: vaesen_teddy_bear_personality_tomie_master.mp4\n");
	printf("=============================================================\n");

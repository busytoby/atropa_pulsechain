...
/*
 * Auncient 100% Pixar USDA Driven Babyface Master Renderer Engine
 * Reads eye, nose, ear, mouth, and personality parameters directly from Pixar USDA scene assets:
 *   - `tests/vaesen_teddy_bear_eyes.usda`
 *   - `tests/vaesen_teddy_bear_noses.usda`
 *   - `tests/vaesen_teddy_bear_ears.usda`
 *   - `tests/vaesen_teddy_bear_mouths.usda`
 *   - `tests/vaesen_teddy_bear_personality_tomie.usda`
 * Evaluates stationary eye & nose positions directly driven by Pixar USDA prim definitions.
 * Outputting directly to 'vaesen_teddy_bear_usda_driven_babyface.mp4'.
 */
...
static double scene_sdf(Vector3 p, double ear_twitch)
{
	...
	/* 100% Pixar USDA Driven Mesh Parameters */
	double head = sdf_sphere(p, (Vector3){ 0.0, head_y, 2.0 }, 0.44 * (global_usda_params.head_fwhr / 1.15));
	...
	/* 100% PIXAR USDA PARSED EYE & NOSE PRIM POSITIONS AND RADII */
	double eye_radius = global_usda_params.eye_radius * (global_usda_params.eye_scale / 1.6);
	double eye_left = sdf_sphere(p, (Vector3){ global_usda_params.eye_left_x, global_usda_params.eye_left_y, global_usda_params.eye_left_z }, eye_radius);
	double eye_right = sdf_sphere(p, (Vector3){ global_usda_params.eye_right_x, global_usda_params.eye_right_y, global_usda_params.eye_right_z }, eye_radius);
	double nose_heart = sdf_sphere(p, (Vector3){ global_usda_params.nose_x, global_usda_params.nose_y, global_usda_params.nose_z }, global_usda_params.nose_radius);
		...
		for (int x = 0; x < WIDTH; x++) {
			...
			if (hit) {
				...
				if (p.y <= -0.91) {
					...
					/* EVALUATE AGAINST PARSED USDA PRIM POSITIONS */
					double eye_dist_l = vec3_length(vec3_sub(p, (Vector3){ global_usda_params.eye_left_x, global_usda_params.eye_left_y, global_usda_params.eye_left_z }));
					double eye_dist_r = vec3_length(vec3_sub(p, (Vector3){ global_usda_params.eye_right_x, global_usda_params.eye_right_y, global_usda_params.eye_right_z }));
					double nose_dist = vec3_length(vec3_sub(p, (Vector3){ global_usda_params.nose_x, global_usda_params.nose_y, global_usda_params.nose_z }));

					if (eye_dist_l < (global_usda_params.eye_radius + 0.002) || eye_dist_r < (global_usda_params.eye_radius + 0.002)) {
					...
					} else if (nose_dist < (global_usda_params.nose_radius + 0.002)) {
...
{
	printf("=============================================================\n");
	printf("100%% PIXAR USDA DRIVEN BABYFACE MASTER RENDER ENGINE         \n");
	printf("=============================================================\n");
	...
	printf("   ✓ Parsed USDA Eyes: Radius %.4f m | Left (%.2f, %.2f, %.2f) | Right (%.2f, %.2f, %.2f)\n",
	       global_usda_params.eye_radius, global_usda_params.eye_left_x, global_usda_params.eye_left_y, global_usda_params.eye_left_z,
	       global_usda_params.eye_right_x, global_usda_params.eye_right_y, global_usda_params.eye_right_z);
	printf("   ✓ Parsed USDA Nose: Radius %.4f m | Position (%.2f, %.2f, %.2f)\n",
	       global_usda_params.nose_radius, global_usda_params.nose_x, global_usda_params.nose_y, global_usda_params.nose_z);

	FILE *ffmpeg = popen("ffmpeg -y -f rawvideo -vcodec rawvideo -s 1280x720 -pix_fmt rgb24 -r 30 -i - -c:v libx264 -pix_fmt yuv420p vaesen_teddy_bear_usda_driven_babyface.mp4 > /dev/null 2>&1", "w");
	if (!ffmpeg) {
		fprintf(stderr, "Error spawning ffmpeg encoder pipeline.\n");
		return 1;
	}

	printf("Rendering %d Pixar USDA Driven Babyface frames across 16 parallel threads...\n", TOTAL_FRAMES);
	...
	printf("=============================================================\n");
	printf("   PIXAR USDA DRIVEN RENDER COMPLETE: vaesen_teddy_bear_usda_driven_babyface.mp4\n");
	printf("=============================================================\n");

...
/*
 * Auncient Fixed-Anatomy Expressive 3D Vaesen Teddy Bear Renderer Engine
 * Fixes eye & facial anatomy alignment:
 * 1. Eyes are strictly stationary relative to the head mesh origin.
 * 2. Anatomic proportional eye-to-nose layout:
 *    - Nose tip: (0.0, 0.16, 1.50)
 *    - Left Eye: (-0.17, 0.28, 1.62)
 *    - Right Eye: (0.17, 0.28, 1.62)
 *    - Left Eyebrow: (-0.17, 0.38, 1.62)
 *    - Right Eyebrow: (0.17, 0.38, 1.62)
 * 3. Pupil gaze shifts internally while outer glass sphere remains anchored.
 * 4. 16-Thread POSIX Multi-Core Acceleration outputting directly to 'vaesen_teddy_bear_fixed_anatomy.mp4'.
 */
		...
		for (int x = 0; x < WIDTH; x++) {
			...
			if (hit) {
				...
				if (p.y <= -0.91) {
					...
					/* STATIONARY ANATOMY RELATIVE TO HEAD ORIGIN */
					/* 1. Outer Glass Eye Spheres (Stationary) */
					double eye_lx = -0.17;
					double eye_ly = head_y + 0.03; /* 0.28 when head_y is 0.25 */
					double eye_lz = 1.62;

					double eye_rx = 0.17;
					double eye_ry = head_y + 0.03;
					double eye_rz = 1.62;

					double eye_dist_l = vec3_length(vec3_sub(p, (Vector3){ eye_lx, eye_ly, eye_lz }));
					double eye_dist_r = vec3_length(vec3_sub(p, (Vector3){ eye_rx, eye_ry, eye_rz }));

					/* 2. Sculpted Button Velvet Nose (Stationary) */
					double nose_dist = vec3_length(vec3_sub(p, (Vector3){ 0.0, head_y - 0.09, 1.50 }));

					/* 3. Eyebrow Arches (Stationary) */
					double brow_lx = p.x - eye_lx;
					double brow_ly = p.y - (eye_ly + 0.10);
					double brow_rx = p.x - eye_rx;
					double brow_ry = p.y - (eye_ry + 0.10);
					...
					/* 4. Smiling Mouth Line (Stationary) */
					double mouth_x = p.x;
					double mouth_y = p.y - (head_y - 0.23);
					...
					if (eye_dist_l < 0.070 || eye_dist_r < 0.070) {
						/* STATIONARY GLASS EYE SPHERES WITH INTERNAL PUPIL SHIFT */
						...
						/* Internal Pupil Gaze Shifting (Outer Sphere Remains Anchored) */
						double active_eye_x = (eye_dist_l < 0.070) ? eye_lx : eye_rx;
						double pupil_dist = vec3_length(vec3_sub(p, (Vector3){ active_eye_x + args->eye_gaze_x, eye_ly + args->eye_gaze_y, 1.58 }));

						if (pupil_dist < 0.025) {
							set_pixel(x, y, 255, 255, 255); /* White Specular Catchlight */
						} else {
						...
						set_pixel(x, y, (uint8_t)(r * 255.0f), (uint8_t)(g * 255.0f), (uint8_t)(b * 255.0f));
					} else if (mouth_dist < 0.016 && p.y < (head_y - 0.15) && fabs(p.x) < 0.11) {
...
{
	printf("=============================================================\n");
	printf("FIXED-ANATOMY STATIONARY EYE VAESEN TEDDY BEAR ENGINE        \n");
	printf("=============================================================\n");

	FILE *ffmpeg = popen("ffmpeg -y -f rawvideo -vcodec rawvideo -s 1280x720 -pix_fmt rgb24 -r 30 -i - -c:v libx264 -pix_fmt yuv420p vaesen_teddy_bear_fixed_anatomy.mp4 > /dev/null 2>&1", "w");
	if (!ffmpeg) {
		fprintf(stderr, "Error spawning ffmpeg encoder pipeline.\n");
		return 1;
	}

	printf("Rendering %d Fixed-Anatomy frames across 16 parallel threads...\n", TOTAL_FRAMES);
	...
	for (int frame = 0; frame < TOTAL_FRAMES; frame++) {
		...
		/* Internal Pupil Gaze Shifting (Outer Eye Spheres Remain Anchored) */
	...
	printf("=============================================================\n");
	printf("   FIXED-ANATOMY RENDER COMPLETE: vaesen_teddy_bear_fixed_anatomy.mp4\n");
	printf("=============================================================\n");

...
int main(void) {
    system("cp /tmp/cbt001/CBT001.zip /tmp/cbt001/cbt001_info.dat.bin");

    FILE *f = tsfi_tape_open_guarded("/tmp/cbt001/cbt001_info.dat.bin", "rb", TAPE_SECURITY_UNCLASSIFIED);
    if (!f) {
    ...
    uint8_t card[80];
    char ascii[81];
    bool in_file_2 = false;

    printf("\n=== EXTRACTING FILE 002 DOCUMENTATION ===\n");
    ...
    while (fread(card, 1, 80, f) == 80) {
        ...
        if (strncmp(ascii, "//***FILE", 9) == 0) {
            // Replace non-printable character at index 9 with space if needed
            if (!isdigit((unsigned char)ascii[9])) {
                ascii[9] = ' ';
            }
            int file_num = 0;
            if (sscanf(ascii + 9, "%d", &file_num) == 1) {
                if (file_num == 2) {
                    in_file_2 = true;
                } else {
                    in_file_2 = false;
    ...
    pclose(f);
    remove("/tmp/cbt001/cbt001_info.dat.bin");
    return 0;
}

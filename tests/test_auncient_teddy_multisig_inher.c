...
// Formally verifies:
// 1. solidity/dysnomia/domain/strategies/auncient_teddy_multisig_inher.strategy (COBOL)
// 2. solidity/dysnomia/domain/std/auncient_teddy_multisig_inher_prover.algol61 (Algol61)

static int algol61_verify_teddy_multisig(
...
int main(void) {
    ...
    // Test 1: Clean Teddy Multi-Sig Verification (MultiSig Jitter=13500 ps, Signatories M=8, N=16)
    uint64_t clean_out = 0;
    uint32_t clean_disp = 0;
    int clean_ruling = algol61_verify_teddy_multisig(
        13500ULL /* MultiSig Jitter */, 8 /* Signatory Nodes M=8 */, 16 /* Cycles */, 555 /* TeddyBear */, false, 3, &clean_out, &clean_disp
    );
    assert(clean_ruling == 0);
    printf("   ✓ Clean Teddy Multi-Sig Verification verified (Ruling=0, Out=%lu, DispMod=%u).\n",
    ...
    for (uint32_t nodes = 1; nodes <= 16; nodes += 3) {
        ...
        int sweep_ruling = algol61_verify_teddy_multisig(
    ...
    int fault_ruling = algol61_verify_teddy_multisig(
    ...
    assert(algol61_verify_teddy_multisig(13500ULL, 8, 16, 555, false, 4 /* k!=3 */, NULL, NULL) == 1);
    assert(algol61_verify_teddy_multisig(50001ULL /* jitter>50k */, 8, 16, 555, false, 3, NULL, NULL) == 2);
    assert(algol61_verify_teddy_multisig(13500ULL, 0 /* M=0 */, 16, 555, false, 3, NULL, NULL) == 3);
    assert(algol61_verify_teddy_multisig(13500ULL, 17 /* M>16 */, 16, 555, false, 3, NULL, NULL) == 3);
    assert(algol61_verify_teddy_multisig(13500ULL, 8, 65 /* N>64 */, 555, false, 3, NULL, NULL) == 4);
    assert(algol61_verify_teddy_multisig(13500ULL, 8, 16, 1234 /* unverified */, false, 3, NULL, NULL) == 5);

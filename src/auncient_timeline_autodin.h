...
typedef struct {
    uint32_t account_id;
    uint8_t type; // 0 = OPEN/ENDOWMENT, 1 = SEND, 2 = RECEIVE, 3 = CHANGE_REP
    uint32_t balance;
    uint32_t amount;
    uint32_t sequence;
...
// Dispatches a resolved Initial Orders 2 instruction to a receiver target via WinchesterMQ (wmq). Requires PKI keys count >= 4.

...
1. Which binaries implement the "teddy bear editor" and "drum sequencer" components? We see references to `tsfi_teddy_bear_editor`, `test_vulkan_teddy_bear`, and `test_wayland_terminal_shell` (which contains Rhythmic Bits step sequencer).
...
3. How is the drum sequencer structured? If it is inside `test_wayland_terminal_shell.c`, how does it play audio (or does it just print text)? If it's inside `test_vulkan_teddy_bear.c`, how is ALSA audio initialized, mixed, and played?

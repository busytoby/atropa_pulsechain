...
namespace Dysnomia
{
    public class ညြ
    {
        ...
        public ညြ(bool RAPHE_NEBULAE)
        {
            ...
            Mu.Tau.WaitOne();
            while (Mu.Chi < 2) { Mu.Tau.ReleaseMutex(); Thread.Sleep(2000); Mu.Tau.WaitOne(); }
            ...
            Eta.Iota();
            Mu.Tau.WaitOne();
            while (Mu.Chi < 3) { Mu.Tau.ReleaseMutex(); Thread.Sleep(2000); Mu.Tau.WaitOne(); }
        ...
        public ညြ(ref ည Beta, ref ည Iota, ref ည Omicron)
        {
            ...
            Theta = Omicron;
            Mu.Tau.WaitOne();
            while (Mu.Chi < 5) { Mu.Tau.ReleaseMutex(); Thread.Sleep(2000); Mu.Tau.WaitOne(); }
            Theta.Tau.WaitOne();
            while (Theta.Chi < 5) { Theta.Tau.ReleaseMutex(); Thread.Sleep(2000); Theta.Tau.WaitOne(); }

...
function writeLiveConfig(text, colorScheme = -1, shapeType = -1, speedScale = 1.0) {
    ...
    console.log(`[COAXIAL MODULATION] Wrote live config: Text="${text}", Color=${colorScheme}, Shape=${shapeType}, Speed=${speedScale}`);
}

const args = process.argv.slice(2);
if (args.length === 0) {
    console.log("Usage: node scripts/modulate_scroller.js <text> [color_scheme] [shape_type] [speed_scale]");
    process.exit(1);
}
...
writeLiveConfig(text, colorScheme, shapeType, speedScale);
